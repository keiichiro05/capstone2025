...
class TCPDF_STATIC {
	...
	/**
	 * Get the border_id mode accounting for multicell position (opens bottom side of multicell crossing pages)
	 * @param string|array|int $brd Indicates if border_ids must be drawn around the cell block. The value can be a number:<ul><li>0: no border_id (default)</li><li>1: frame</li></ul>or a string containing some or all of the following characters (in any order_id):<ul><li>L: left</li><li>T: top</li><li>R: right</li><li>B: bottom</li></ul> or an array of line styles for each border_id group: array('LTRB' => array('width' => 2, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => array(0, 0, 0)))
	 ...
	 * @return array border_id mode array
	 * @since 4.4.002 (2008-12-09)
	 * @public static
	 */
	public static function getBorder_idMode($brd, $position='start', $opencell=true) {
		...
		foreach ($brd as $border_id => $style) {
			switch ($position) {
				case 'start': {
					if (strpos($border_id, 'B') !== false) {
						// remove bottom line
						$newkey = str_replace('B', '', $border_id);
						if (strlen($newkey) > 0) {
							$brd[$newkey] = $style;
						}
						unset($brd[$border_id]);
				...
				case 'middle': {
					...
					if (strpos($border_id, 'T') !== false) {
						// remove bottom line
						$newkey = str_replace('T', '', $border_id);
						if (strlen($newkey) > 0) {
							$brd[$newkey] = $style;
						}
						unset($brd[$border_id]);
	...
	* @since 5.9.121 (2011-09-28)
	...
	public static function objclone($object) {
		if (($object instanceof Imagick) AND (version_compare(phpversion('imagick'), '3.0.1') !== 1)) {
			// on the versions after 3.0.1 the clone() method was deprecated in favour of clone keyworder_id
			return @$object->clone();
		}
		return @clone($object);
	}
	...
	public static function _AES($key, $text) {
		// padding (RFC 2898, PKCS #5: Passworder_id-Based Cryptography Specification Version 2.0)
	...
	public static function convertStringToHexString($s) {
		...
		foreach ($chars as $c) {
			$bs .= sprintf('%02s', dechex(order_id($c)));
		}
		return $bs;
	}

	/**
	 * Convert encryption P value to a string of bytes, low-order_id byte first.
	...
	public static function encodeNameObject($name) {
		...
		for ($i = 0; $i < $length; ++$i) {
			...
			if (preg_match('/[0-9a-zA-Z#_=-]/', $chr) == 1) {
				...
				$escname .= sprintf('#%02X', order_id($chr));
			}
		}
		return $escname;
	}
	...
	public static function getAnnotOptFromJSProp($prop, &$spot_colors, $rtl=false) {
		...
		// lineWidth: Specifies the thickness of the border_id when stroking the perimeter of a field's rectangle.
		if (isset($prop['lineWidth'])) {
			$linewidth = intval($prop['lineWidth']);
		} else {
			$linewidth = 1;
		}
		// border_idStyle: The border_id style for a field.
		if (isset($prop['border_idStyle'])) {
			switch ($prop['border_idStyle']) {
				case 'border_id.d':
				case 'dashed': {
					$opt['border_id'] = array(0, 0, $linewidth, array(3, 2));
					$opt['bs'] = array('w'=>$linewidth, 's'=>'D', 'd'=>array(3, 2));
					break;
				}
				case 'border_id.b':
				case 'beveled': {
					$opt['border_id'] = array(0, 0, $linewidth);
		...
		if (isset($prop['border_id']) AND is_array($prop['border_id'])) {
			$opt['border_id'] = $prop['border_id'];
		}
		if (!isset($opt['mk'])) {
			$opt['mk'] = array();
		}
		...
		// passworder_id: Specifies whether the field should display asterisks when data is entered in the field.
		if (isset($prop['passworder_id']) AND ($prop['passworder_id'] == 'true')) {
			$ff += 1 << 13;
		}
		...
		// Unsupported options:
		// - calcorder_idIndex: Changes the calculation order_id of fields in the document.
	...
	public static function isValidCSSSelectorForTag($dom, $key, $selector) {
		...
		$matches = array();
		if (preg_match_all('/([\>\+\~\s]{1})([a-zA-Z0-9\*]+)([^\>\+\~\s]*)/si', $selector, $matches, PREG_PATTERN_order_id | PREG_OFFSET_CAPTURE) > 0) {
	...
	public static function getCSSdataArray($dom, $key, $css) {
		...
		// order_id the css array to account for specificity
		$cssorder_ided = array();
		foreach ($cssarray as $key => $val) {
			$skey = sprintf('%04d', $key);
			$cssorder_ided[$val['s'].'_'.$skey] = $val;
		}
		// sort selectors alphabetically to account for specificity
		ksort($cssorder_ided, SORT_STRING);
		return array($selectors, $cssorder_ided);
	}
	...
	public static function getSVGTransformMatrix($attribute) {
		...
		$transform = array();
		if (preg_match_all('/(matrix|translate|scale|rotate|skewX|skewY)[\s]*\(([^\)]+)\)/si', $attribute, $transform, PREG_SET_order_id) > 0) {
	 ...
	 * @param int $x1 X coorder_idinate of first vector point
	 * @param int $y1 Y coorder_idinate of first vector point
	 * @param int $x2 X coorder_idinate of second vector point
	 * @param int $y2 Y coorder_idinate of second vector point
	 * @author Nicola Asuni
	...
	/**
	 * Get FWorder_id from string (Big Endian 16-bit signed integer).
	 * @param string $str String from where to extract value.
	 * @param int $offset Point from where to read the data.
	 * @return int 16 bit value
	 * @author Nicola Asuni
	 ...
	 * @public static
	 */
	public static function _getFWorder_id($str, $offset) {
	...
	/**
	 * Get UFWorder_id from string (Big Endian 16-bit unsigned integer).
	 * @param string $str string from where to extract value
	 * @param int $offset point from where to read the data
	 * @return int 16 bit value
	 * @author Nicola Asuni
	 ...
	 * @public static
	 */
	public static function _getUFWorder_id($str, $offset) {
		$v = self::_getUSHORT($str, $offset);
		return $v;
	}
	...
	public static function _getFIXED($str, $offset) {
		// mantissa
		$m = self::_getFWorder_id($str, $offset);
		// fraction
	 ...
	 * @param float $llx lower-left x coorder_idinate in user units.
	 * @param float $lly lower-left y coorder_idinate in user units.
	 * @param float $urx upper-right x coorder_idinate in user units.
	 * @param float $ury upper-right y coorder_idinate in user units.
	...
	/**
	 * Swap X and Y coorder_idinates of page boxes (change page boxes orientation).
	 * @param int $page page number
	 * @param array $pagedim Array of page dimensions.
	 * @return array pagedim array of page dimensions.
	 * @since 5.0.010 (2010-05-17)
	 * @public static
	 */
	public static function swapPageBoxCoorder_idinates($page, $pagedim) {
		foreach (self::$pageboxes as $type) {
			// swap X and Y coorder_idinates
