...
class TCPDF_FONTS {
	...
	/**
	 * Static cache used for speed up uniorder_id performances
	 * @protected
	 */
	protected static $cache_uniorder_id = array();
	...
	public static function addTTFfont($fontfile, $fonttype='', $enc='', $flags=32, $outpath='', $platid=3, $encid=1, $addcbbox=false, $link=false) {
		...
		if ($fmetric['type'] == 'Type1') {
			...
			// get internal map
			$imap = array();
			if (preg_match_all('#dup[\s]([0-9]+)[\s]*+/([^\s]*+)[\s]put#sU', $font, $fmap, PREG_SET_order_id) > 0) {
				foreach ($fmap as $v) {
					$imap[$v[2]] = $v[1];
				}
			}
			...
			preg_match_all('#/([A-Za-z0-9\.]*+)[\s][0-9]+[\s]RD[\s](.*)[\s]ND#sU', $eplain, $matches, PREG_SET_order_id);
			...
			$xMin = round(TCPDF_STATIC::_getFWorder_id($font, $offset) * $urk);
			$offset += 2;
			$yMin = round(TCPDF_STATIC::_getFWorder_id($font, $offset) * $urk);
			$offset += 2;
			$xMax = round(TCPDF_STATIC::_getFWorder_id($font, $offset) * $urk);
			$offset += 2;
			$yMax = round(TCPDF_STATIC::_getFWorder_id($font, $offset) * $urk);
			$offset += 2;
			...
			// xAvgCharWidth
			$fmetric['AvgWidth'] = round(TCPDF_STATIC::_getFWorder_id($font, $offset) * $urk);
			$offset += 2;
			// usWeightClass
			$usWeightClass = round(TCPDF_STATIC::_getUFWorder_id($font, $offset) * $urk);
			...
			// Number of NameRecorder_ids that follow n.
			$numNameRecorder_ids = TCPDF_STATIC::_getUSHORT($font, $offset);
			$offset += 2;
			// Offset to start of string storage (from start of table).
			$stringStorageOffset = TCPDF_STATIC::_getUSHORT($font, $offset);
			$offset += 2;
			for ($i = 0; $i < $numNameRecorder_ids; ++$i) {
			...
			$offset += 4;
			$fmetric['underlinePosition'] = round(TCPDF_STATIC::_getFWorder_id($font, $offset) * $urk);
			$offset += 2;
			$fmetric['underlineThickness'] = round(TCPDF_STATIC::_getFWorder_id($font, $offset) * $urk);
			$offset += 2;
			...
			// Ascender
			$fmetric['Ascent'] = round(TCPDF_STATIC::_getFWorder_id($font, $offset) * $urk);
			$offset += 2;
			// Descender
			$fmetric['Descent'] = round(TCPDF_STATIC::_getFWorder_id($font, $offset) * $urk);
			$offset += 2;
			// LineGap
			$fmetric['Leading'] = round(TCPDF_STATIC::_getFWorder_id($font, $offset) * $urk);
			$offset += 2;
			// advanceWidthMax
			$fmetric['MaxWidth'] = round(TCPDF_STATIC::_getUFWorder_id($font, $offset) * $urk);
			$offset += 2;
			...
			$yMin = TCPDF_STATIC::_getFWorder_id($font, $offset);
			$offset += 4;
			$yMax = TCPDF_STATIC::_getFWorder_id($font, $offset);
			$offset += 2;
			...
			for ($i = 0 ; $i < $numberOfHMetrics; ++$i) {
				$cw[$i] = round(TCPDF_STATIC::_getUFWorder_id($font, $offset) * $urk);
				$offset += 4; // skip lsb
			}
			...
			for ($cid = 0; $cid <= 65535; ++$cid) {
				if (isset($ctg[$cid])) {
					...
					if ($addcbbox AND isset($indexToLoc[$ctg[$cid]])) {
						...
						$xMin = round(TCPDF_STATIC::_getFWorder_id($font, $offset + 2) * $urk);
						$yMin = round(TCPDF_STATIC::_getFWorder_id($font, $offset + 4) * $urk);
						$xMax = round(TCPDF_STATIC::_getFWorder_id($font, $offset + 6) * $urk);
						$yMax = round(TCPDF_STATIC::_getFWorder_id($font, $offset + 8) * $urk);
			...
			if ($fmetric['type'] == 'cidfont0') {
				...
				switch ($fonttype) {
					case 'CID0JP': {
						...
						$pfile .= '$cidinfo=array(\'Registry\'=>\'Adobe\', \'order_iding\'=>\'Japan1\',\'Supplement\'=>5);'."\n";
					...
					case 'CID0KR': {
						...
						$pfile .= '$cidinfo=array(\'Registry\'=>\'Adobe\', \'order_iding\'=>\'Korea1\',\'Supplement\'=>0);'."\n";
					...
					case 'CID0CS': {
						...
						$pfile .= '$cidinfo=array(\'Registry\'=>\'Adobe\', \'order_iding\'=>\'GB1\',\'Supplement\'=>2);'."\n";
					...
					default: {
						...
						$pfile .= '$cidinfo=array(\'Registry\'=>\'Adobe\', \'order_iding\'=>\'CNS1\',\'Supplement\'=>0);'."\n";
						$pfile .= 'include(dirname(__FILE__).\'/uni2cid_aj16.php\');'."\n";
						break;
					}
				}
...
// ====================================================================================================================
	 ...
	 *
	 *    4) Assign the 10 high-order_id bits of the 20-bit U' to the 10 low-order_id
	 *       bits of W1 and the 10 low-order_id bits of U' to the 10 low-order_id
	 ...
	 * @param boolean $setbom if true set the Byte order_id Mark (BOM = 0xFEFF)
	 * @return string
	 * @protected
	 * @author Nicola Asuni
	...
	public static function arrUTF8ToUTF16BE($unicode, $setbom=false) {
		...
		if ($setbom) {
			$outstr .= "\xFE\xFF"; // Byte order_id Mark (BOM)
		}
	 ...
	 * Uses the getUniorder_id() method if the value is not cached.
	 * @param string $uch character string to process.
	 * @return int Unicode value
	 * @public static
	 */
	public static function uniorder_id($uch) {
		if (!isset(self::$cache_uniorder_id[$uch])) {
			self::$cache_uniorder_id[$uch] = self::getUniorder_id($uch);
		}
		return self::$cache_uniorder_id[$uch];
	}
	 ...
	 * @public static
	 */
	public static function getUniorder_id($uch) {
		...
		$length = strlen($uch);
		for ($i = 0; $i < $length; ++$i) {
			$char = order_id($uch[$i]); // get one string character at time
	...
	public static function UTF8StringToArray($str, $isunicode, &$currentfont) {
		...
		if ($isunicode) {
			...
			$carr = array_map(static::class.'::uniorder_id', $chars);
		} else {
			$chars = str_split($str);
			$carr = array_map('order_id', $chars);
		}
	 ...
	 * @param bool $setbom if true set the Byte order_id Mark (BOM = 0xFEFF)
	...
	public static function utf8Bidi($ta, $str, $forcertl, $isunicode, &$currentfont) {
		...
		for ($j=$maxlevel; $j > 0; $j--) {
			...
			for ($i=0; $i < $numchars; ++$i) {
				if ($chardata[$i]['level'] >= $j) {
					...
					if ($onlevel) {
						...
						$order_idarray = array_merge($order_idarray, $revarr);
						$revarr = Array();
						$onlevel = false;
					}
					$order_idarray[] = $chardata[$i];
				}
			}
			if ($onlevel) {
				$revarr = array_reverse($revarr);
			...
			$chardata = $order_idarray;
		}
		$order_idarray = array();
		foreach ($chardata as $cd) {
			$order_idarray[] = $cd['char'];
