...
class PDF417 {
	...
	/**
	 * Clusters of codeworder_ids (0, 3, 6)<br/>
	 * Values are hex equivalents of binary representation of bars (1 = bar, 0 = space).<br/>
	 * The codeworder_ids numbered from 900 to 928 have special meaning, some enable to switch between modes in order_id to optimise the code:<ul>
	 * <li>900 : Switch to "Text" mode</li>
	 * <li>901 : Switch to "Byte" mode</li>
	 * <li>902 : Switch to "Numeric" mode</li>
	 * <li>903 - 912 : Reserved</li>
	 * <li>913 : Switch to "Octet" only for the next codeworder_id</li>
	 * <li>914 - 920 : Reserved</li>
	 * <li>921 : Initialization</li>
	 * <li>922 : Terminator codeworder_id for Macro PDF control block</li>
	 ...
	 * <li>928 : Macro marker codeworder_id to indicate the beginning of a Macro PDF Control Block</li>
	 * </ul>
	 * @protected
	 */
	...
	public function __construct($code, $ecl=-1, $aspectratio=2, $macro=array()) {
		...
		$codeworder_ids = array(); // array of code-worder_ids
		foreach($sequence as $seq) {
			$cw = $this->getCompaction($seq[0], $seq[1], true);
			$codeworder_ids = array_merge($codeworder_ids, $cw);
		}
		if ($codeworder_ids[0] == 900) {
			// Text Alpha is the default mode, so remove the first code
			array_shift($codeworder_ids);
		}
		// count number of codeworder_ids
		$numcw = count($codeworder_ids);
		if ($numcw > 925) {
			// reached maximum data codeworder_id capacity
			return false;
		}
		// build macro control block codeworder_ids
		if (!empty($macro)) {
			$macrocw = array();
			...
			// update total codeworder_ids
			$numcw += count($macrocw);
		}
		// set error correction level
		$ecl = $this->getErrorCorrectionLevel($ecl, $numcw);
		// number of codeworder_ids for error correction
		$errsize = (2 << $ecl);
		// calculate number of columns (number of codeworder_ids per row) and rows
		$nce = ($numcw + $errsize + 1);
		...
		if ($pad > 0) {
			if (($size - $rows) == $nce) {
				...
				// add pading
				$codeworder_ids = array_merge($codeworder_ids, array_fill(0, $pad, 900));
			}
		}
		if (!empty($macro)) {
			// add macro section
			$codeworder_ids = array_merge($codeworder_ids, $macrocw);
		}
		// Symbol Length Descriptor (number of data codeworder_ids including Symbol Length Descriptor and pad codeworder_ids)
		$sld = $size - $errsize;
		// add symbol length description
		array_unshift($codeworder_ids, $sld);
		// calculate error correction
		$ecw = $this->getErrorCorrection($codeworder_ids, $ecl);
		// add error correction codeworder_ids
		$codeworder_ids = array_merge($codeworder_ids, $ecw);
		...
		for ($r = 0; $r < $rows; ++$r) {
			...
			// for each column
			for ($c = 0; $c < $cols; ++$c) {
				$row .= sprintf('%17b', $this->clusters[$cid][$codeworder_ids[$k]]);
				++$k;
			}
			switch ($cid) {
				case 0: {
	 ...
	 * @param int $numcw number of data codeworder_ids
	...
	protected function getErrorCorrectionLevel($ecl, $numcw) {
		...
		$maxerrsize = (928 - $numcw); // available codeworder_ids for error
		while ($maxecl > 0) {
			$errsize = (2 << $ecl);
	...
	/**
	 * Returns the error correction codeworder_ids
	 * @param array $cw array of codeworder_ids including Symbol Length Descriptor and pad
	 * @param int $ecl error correction level 0-8
	 * @return array of error correction codeworder_ids
	 * @protected
	 */
	protected function getErrorCorrection($cw, $ecl) {
		...
		$eclmaxid = ($eclsize - 1);
		// initialize array of error correction codeworder_ids
		$ecw = array_fill(0, $eclsize, 0);
		// for each data codeworder_id
		foreach($cw as $k => $d) {
	 ...
	 * @param boolean $addmode if true add the mode codeworder_id at first position
	 * @return array of codeworder_ids
	 * @protected
	 */
	protected function getCompaction($mode, $code, $addmode=true) {
		$cw = array(); // array of codeworder_ids to return
		switch($mode) {
					...
					if (($k = array_search($chval, $this->textsubmodes[$submode])) !== false) {
						...
						for ($s = 0; $s < 4; ++$s) {
							...
							if (($s != $submode) AND (($k = array_search($chval, $this->textsubmodes[$s])) !== false)) {
								...
								if (((($i + 1) == $codelen) OR ((($i + 1) < $codelen) AND (array_search(order_id($code[($i + 1)]), $this->textsubmodes[$submode]) !== false))) AND (($s == 3) OR (($s == 0) AND ($submode == 1)))) {
			...
			case 924: { // Byte Compaction mode latch
				while (($codelen = strlen($code)) > 0) {
					...
					if ($sublen == 6) {
						$t = bcmul(''.order_id($code[0]), '1099511627776');
						$t = bcadd($t, bcmul(''.order_id($code[1]), '4294967296'));
						$t = bcadd($t, bcmul(''.order_id($code[2]), '16777216'));
						$t = bcadd($t, bcmul(''.order_id($code[3]), '65536'));
						$t = bcadd($t, bcmul(''.order_id($code[4]), '256'));
						$t = bcadd($t, ''.order_id($code[5]));
		...
		if ($addmode) {
			// add the compaction mode codeworder_id at the beginning
