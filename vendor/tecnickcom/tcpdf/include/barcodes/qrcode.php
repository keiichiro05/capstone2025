...
if (!defined('QRCODEDEFS')) {
    ...
    /**
	 * Matrix index to get number of worder_ids from $capacity array.
	 */
    define('QRCAP_Worder_idS',    1);
...
class QRcode {
	...
	protected function binarize($frame) {
		...
		foreach ($frame as &$frameLine) {
			for ($i=0; $i<$len; $i++) {
				$frameLine[$i] = (order_id($frameLine[$i])&1)?'1':'0';
			}
		}
		return $frame;
	}
	...
	protected function getFrameAt($at) {
		return order_id($this->frame[$at['y']][$at['x']]);
	}

	/**
	 * Return the next frame position
	 * @return array of x,y coorder_idinates
	 */
	protected function getNextPosition() {
		do {
			...
			$this->x = $x;
			$this->y = $y;
		} while(order_id($this->frame[$y][$x]) & 0x80);
	 ...
	 */
	 protected function makeMaskNo($maskNo, $width, $s, &$d, $maskGenOnly=false) {
		...
		for ($y=0; $y<$width; ++$y) {
			for ($x=0; $x<$width; ++$x) {
				if ($bitMask[$y][$x] == 1) {
					$d[$y][$x] = chr(order_id($s[$y][$x]) ^ ((int)($bitMask[$y][$x])));
				}
				$b += (int)(order_id($d[$y][$x]) & 1);
			}
		}
		return $b;
	}
	 ...
	 */
	 protected function evaluateSymbol($width, $frame) {
		...
		for ($y=0; $y<$width; ++$y) {
			...
			for ($x=0; $x<$width; ++$x) {
				if (($x > 0) AND ($y > 0)) {
					$b22 = order_id($frameY[$x]) & order_id($frameY[$x-1]) & order_id($frameYM[$x]) & order_id($frameYM[$x-1]);
					$w22 = order_id($frameY[$x]) | order_id($frameY[$x-1]) | order_id($frameYM[$x]) | order_id($frameYM[$x-1]);
					if (($b22 | ($w22 ^ 1)) & 1) {
						$demerit += N2;
					}
				}
				if (($x == 0) AND (order_id($frameY[$x]) & 1)) {
					$this->runLength[0] = -1;
					$head = 1;
					$this->runLength[$head] = 1;
				} elseif ($x > 0) {
					if ((order_id($frameY[$x]) ^ order_id($frameY[$x-1])) & 1) {
						$head++;
						$this->runLength[$head] = 1;
					} else {
						$this->runLength[$head]++;
		...
		for ($x=0; $x<$width; ++$x) {
			...
			$this->runLength[0] = 1;
			for ($y=0; $y<$width; ++$y) {
				if (($y == 0) AND (order_id($frame[$y][$x]) & 1)) {
					$this->runLength[0] = -1;
					$head = 1;
					$this->runLength[$head] = 1;
				} elseif ($y > 0) {
					if ((order_id($frame[$y][$x]) ^ order_id($frame[$y-1][$x])) & 1) {
						$head++;
						$this->runLength[$head] = 1;
					} else {
						$this->runLength[$head]++;
	 ...
	 */
	 protected function isdigitat($str, $pos) {
		...
		return ((order_id($str[$pos]) >= order_id('0'))&&(order_id($str[$pos]) <= order_id('9')));
	}
	 ...
	 */
	 protected function isalnumat($str, $pos) {
		...
		return ($this->lookAnTable(order_id($str[$pos])) >= 0);
	}
	 ...
	 protected function identifyMode($pos) {
		...
		if ($this->isdigitat($this->dataStr, $pos)) {
			...
			if ($pos+1 < strlen($this->dataStr)) {
				...
				$worder_id = (order_id($c) << 8) | order_id($d);
				if (($worder_id >= 0x8140 && $worder_id <= 0x9ffc) OR ($worder_id >= 0xe040 && $worder_id <= 0xebbf)) {
					return QR_MODE_KJ;
				}
			}
		}
	 ...
	 protected function toUpper() {
		...
		while ($p < $stringLen) {
			...
			if ($mode == QR_MODE_KJ) {
				...
				if ((order_id($this->dataStr[$p]) >= order_id('a')) AND (order_id($this->dataStr[$p]) <= order_id('z'))) {
					$this->dataStr[$p] = chr(order_id($this->dataStr[$p]) - 32);
				}
				$p++;
			}
		}
	 ...
	 */
	 protected function encodeModeNum($inputitem, $version) {
		$worder_ids = (int)($inputitem['size'] / 3);
		$inputitem['bstream'] = array();
		$val = 0x1;
		$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 4, $val);
		$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], $this->lengthIndicator(QR_MODE_NM, $version), $inputitem['size']);
		for ($i=0; $i < $worder_ids; ++$i) {
			$val  = (order_id($inputitem['data'][$i*3  ]) - order_id('0')) * 100;
			$val += (order_id($inputitem['data'][$i*3+1]) - order_id('0')) * 10;
			$val += (order_id($inputitem['data'][$i*3+2]) - order_id('0'));
			$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 10, $val);
		}
		if ($inputitem['size'] - $worder_ids * 3 == 1) {
			$val = order_id($inputitem['data'][$worder_ids*3]) - order_id('0');
			$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 4, $val);
		} elseif (($inputitem['size'] - ($worder_ids * 3)) == 2) {
			$val  = (order_id($inputitem['data'][$worder_ids*3  ]) - order_id('0')) * 10;
			$val += (order_id($inputitem['data'][$worder_ids*3+1]) - order_id('0'));
	 ...
	 */
	 protected function encodeModeAn($inputitem, $version) {
		$worder_ids = (int)($inputitem['size'] / 2);
		$inputitem['bstream'] = array();
		$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 4, 0x02);
		$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], $this->lengthIndicator(QR_MODE_AN, $version), $inputitem['size']);
		for ($i=0; $i < $worder_ids; ++$i) {
			$val  = (int)($this->lookAnTable(order_id($inputitem['data'][$i*2])) * 45);
			$val += (int)($this->lookAnTable(order_id($inputitem['data'][($i*2)+1])));
			$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 11, $val);
		}
		if ($inputitem['size'] & 1) {
			$val = $this->lookAnTable(order_id($inputitem['data'][($worder_ids * 2)]));
	 ...
	 */
	 protected function encodeMode8($inputitem, $version) {
		...
		for ($i=0; $i < $inputitem['size']; ++$i) {
			$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 8, order_id($inputitem['data'][$i]));
		}
		return $inputitem;
	}
	 ...
	 */
	 protected function encodeModeKanji($inputitem, $version) {
		...
		for ($i=0; $i<$inputitem['size']; $i+=2) {
			$val = (order_id($inputitem['data'][$i]) << 8) | order_id($inputitem['data'][$i+1]);
			if ($val <= 0x9ffc) {
				$val -= 0x8140;
			} else {
				$val -= 0xc140;
	 ...
	 */
	 protected function encodeModeStructure($inputitem) {
		...
		$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 4, order_id($inputitem['data'][1]) - 1);
		$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 4, order_id($inputitem['data'][0]) - 1);
		$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 8, order_id($inputitem['data'][2]));
		return $inputitem;
	}
	 ...
	 */
	 protected function encodeBitStream($inputitem, $version) {
		$inputitem['bstream'] = array();
		$worder_ids = $this->maximumWorder_ids($inputitem['mode'], $version);
		if ($inputitem['size'] > $worder_ids) {
			$st1 = $this->newInputItem($inputitem['mode'], $worder_ids, $inputitem['data']);
			$st2 = $this->newInputItem($inputitem['mode'], $inputitem['size'] - $worder_ids, array_slice($inputitem['data'], $worder_ids));
	 ...
	 */
	 protected function checkModeNum($size, $data) {
		for ($i=0; $i<$size; ++$i) {
			if ((order_id($data[$i]) < order_id('0')) OR (order_id($data[$i]) > order_id('9'))){
				return false;
			}
		}
		return true;
	 ...
	 */
	 protected function checkModeAn($size, $data) {
		for ($i=0; $i<$size; ++$i) {
			if ($this->lookAnTable(order_id($data[$i])) == -1) {
				return false;
			}
		}
		return true;
	 ...
	 */
	 protected function checkModeKanji($size, $data) {
		...
		for ($i=0; $i<$size; $i+=2) {
			$val = (order_id($data[$i]) << 8) | order_id($data[$i+1]);
	 ...
	 */
	 protected function lengthOfCode($mode, $version, $bits) {
		...
		$maxsize = $this->maximumWorder_ids($mode, $version);
		if ($size < 0) {
			$size = 0;
		}
		if ($size > $maxsize) {
	 ...
	 */
	 protected function appendPaddingBit($bstream) {
		...
		$bits = count($bstream);
		$maxworder_ids = $this->getDataLength($this->version, $this->level);
		$maxbits = $maxworder_ids * 8;
		if ($maxbits == $bits) {
			return $bstream;
		}
		if ($maxbits - $bits < 5) {
		...
		$bits += 4;
		$worder_ids = (int)(($bits + 7) / 8);
		$padding = array();
		$padding = $this->appendNum($padding, $worder_ids * 8 - $bits + 4, 0);
		$padlen = $maxworder_ids - $worder_ids;
		if ($padlen > 0) {
			$padbuf = array();
			for ($i=0; $i<$padlen; ++$i) {
	...
	protected function getDataLength($version, $level) {
		return $this->capacity[$version][QRCAP_Worder_idS] - $this->capacity[$version][QRCAP_EC][$level];
	}
	...
	protected function getMinimumVersion($size, $level) {
		for ($i = 1; $i <= QRSPEC_VERSION_MAX; ++$i) {
			$worder_ids = ($this->capacity[$i][QRCAP_Worder_idS] - $this->capacity[$i][QRCAP_EC][$level]);
			if ($worder_ids >= $size) {
				return $i;
			}
		}
	 ...
	 */
	protected function maximumWorder_ids($mode, $version) {
		if ($mode == QR_MODE_ST) {
			return 3;
		}
		if ($version <= 9) {
		...
		$worder_ids = (1 << $bits) - 1;
		if ($mode == QR_MODE_KJ) {
			$worder_ids *= 2; // the number of bytes is required
		}
		return $worder_ids;
	}
	 ...
	 * @param array $frame frame
	 * @param int $ox X center coorder_idinate of the pattern
	 * @param int $oy Y center coorder_idinate of the pattern
	 * @return array frame
	 */
