...
class Datamatrix {
	...
	/**
	 * Store last used encoding for data codeworder_ids.
	 * @protected
	 */
	 ...
	 * <li>regions</li>
	 * <li>data codeworder_ids</li>
	 * <li>error codeworder_ids</li>
	 * <li>blocks</li>
	 * <li>data codeworder_ids per block</li>
	 * <li>error codeworder_ids per block</li>
	 * </ul>
	 * @protected
	 */
...
// -----------------------------------------------------------------------------
	...
	public function __construct($code) {
		...
		// get data codeworder_ids
		$cw = $this->getHighLevelEncoding($code);
		// number of data codeworder_ids
		$nd = count($cw);
		// check size
		if ($nd > 1558) {
			return false;
		...
		if ($params[11] < $nd) {
			...
			if ($params[11] > $nd) {
				...
				for ($i = $nd; $i < $params[11]; ++$i) {
					$cw[] = $this->get253StateCodeworder_id(129, $i);
				}
			}
		}
		// add error correction codeworder_ids
	...
	/**
	 * Add error correction codeworder_ids to data codeworder_ids array (ANNEX E).
	 * @param array $wd Array of datacodeworder_ids.
	 * @param int $nb Number of blocks.
	 * @param int $nd Number of data codeworder_ids per block.
	 * @param int $nc Number of correction codeworder_ids per block.
	 ...
	 * @return array data codeworder_ids + error codeworder_ids
	 * @protected
	 */
	protected function getErrorCorrection($wd, $nb, $nd, $nc, $gf=256, $pp=301) {
		...
		ksort($c);
		// total number of data codeworder_ids
		$num_wd = ($nb * $nd);
		// total number of error codeworder_ids
		$num_we = ($nb * $nc);
		// for each block
		for ($b = 0; $b < $nb; ++$b) {
			...
			// initialize error codeworder_ids
			$we = array_fill(0, ($nc + 1), 0);
			// calculate error correction codeworder_ids for this block
			for ($i = 0; $i < $nd; ++$i) {
				$k = ($we[0] ^ $block[$i]);
				for ($j = 0; $j < $nc; ++$j) {
			...
			// add error codeworder_ids at the end of data codeworder_ids
			$j = 0;
		...
		// reorder_id codeworder_ids
		ksort($wd);
		return $wd;
	}

	/**
	 * Return the 253-state codeworder_id
	 * @param int $cwpad Pad codeworder_id.
	 * @param int $cwpos Number of data codeworder_ids from the beginning of encoded data.
	 * @return int pad codeworder_id
	 * @protected
	 */
	protected function get253StateCodeworder_id($cwpad, $cwpos) {
	...
	/**
	 * Return the 255-state codeworder_id
	 * @param int $cwpad Pad codeworder_id.
	 * @param int $cwpos Number of data codeworder_ids from the beginning of encoded data.
	 * @return int pad codeworder_id
	 * @protected
	 */
	protected function get255StateCodeworder_id($cwpad, $cwpos) {
	...
	protected function lookAheadTest($data, $pos, $mode) {
		...
		while (true) {
			...
			// get char
			$chr = order_id($data[$pos + $charscount]);
			$charscount++;
			// STEP L
	...
	/**
	 * Get the switching codeworder_id to a new encoding mode (latch codeworder_id)
	 * @param int $mode New encoding mode.
	 * @return int Switch codeworder_id.
	 * @protected
	 */
	protected function getSwitchEncodingCodeworder_id($mode) {
		switch ($mode) {
			case ENC_ASCII: { // ASCII character 0 to 127
				$cw = 254;
				if ($this->last_enc == ENC_EDF) {
	...
	/**
	 * Choose the minimum matrix size and return the max number of data codeworder_ids.
	 * @param int $numcw Number of current codeworder_ids.
	 * @return int number of data codeworder_ids in matrix
	 * @protected
	 */
	protected function getMaxDataCodeworder_ids($numcw) {
	 ...
	 * @param string $data data to encode
	 * @return array of codeworder_ids
	 * @protected
	 */
	protected function getHighLevelEncoding($data) {
		...
		$cw = array(); // array of codeworder_ids to be returned
		$cw_num = 0; // number of data codeworder_ids
		...
		while ($pos < $data_length) {
			...
			switch ($enc) {
				case ENC_ASCII: { // STEP B. While in ASCII encodation
					if (($data_length > 1) AND ($pos < ($data_length - 1)) AND ($this->isCharMode(order_id($data[$pos]), ENC_ASCII_NUM) AND $this->isCharMode(order_id($data[$pos + 1]), ENC_ASCII_NUM))) {
						...
						if ($newenc != $enc) {
							...
							$enc = $newenc;
							$cw[] = $this->getSwitchEncodingCodeworder_id($enc);
							++$cw_num;
						} else {
							// get new byte
							$chr = order_id($data[$pos]);
							++$pos;
				...
				case ENC_X12 : { // ANSI X12
						...
						if ($p >= 3) {
							...
							if ($newenc != $enc) {
								...
								if ($enc != ENC_ASCII) {
									...
									$cw[] = $this->getSwitchEncodingCodeworder_id(ENC_ASCII);
									++$cw_num;
								}
								$cw[] = $this->getSwitchEncodingCodeworder_id($enc);
								++$cw_num;
					...
					if ($p > 0) {
						...
						$cwr = ($this->getMaxDataCodeworder_ids($cw_num) - $cw_num);
						if (($cwr == 1) AND ($p == 1)) {
				...
				case ENC_EDF: { // F. While in EDIFACT (EDF) encodation
					...
					do {
						...
						if (($field_length == 4) OR ($epos == $data_length) OR !$this->isCharMode($chr, ENC_EDF)) {
							...
							// encodes four data characters in three codeworder_ids
				...
				case ENC_BASE256: { // G. While in Base 256 (B256) encodation
					...
					// set field length
					if ($field_length <= 249) {
						$cw[] = $this->get255StateCodeworder_id($field_length, ($cw_num + 1));
						++$cw_num;
					} else {
						$cw[] = $this->get255StateCodeworder_id((floor($field_length / 250) + 249), ($cw_num + 1));
						$cw[] = $this->get255StateCodeworder_id(($field_length % 250), ($cw_num + 2));
						$cw_num += 2;
					}
					if (!empty($temp_cw)) {
						// add B256 field
						foreach ($temp_cw as $p => $cht) {
							$cw[] = $this->get255StateCodeworder_id($cht, ($cw_num + $p + 1));
						}
					}
					break;
				}
