...
class TCPDFBarcode {
	...
	protected function barcode_imb($code) {
		...
		// convert binary data to codeworder_ids
		$codeworder_ids = array();
		$data = $this->hex_to_dec($binary_code_102bit);
		$codeworder_ids[0] = bcmod($data, 636) * 2;
		$data = bcdiv($data, 636);
		for ($i = 1; $i < 9; ++$i) {
			$codeworder_ids[$i] = bcmod($data, 1365);
			$data = bcdiv($data, 1365);
		}
		$codeworder_ids[9] = $data;
		if (($fcs >> 10) == 1) {
			$codeworder_ids[9] += 659;
		}
		// generate lookup tables
		$table2of13 = $this->imb_tables(2, 78);
		$table5of13 = $this->imb_tables(5, 1287);
		// convert codeworder_ids to characters
		$characters = array();
		$bitmask = 512;
		foreach($codeworder_ids as $k => $val) {
			if ($val <= 1286) {
