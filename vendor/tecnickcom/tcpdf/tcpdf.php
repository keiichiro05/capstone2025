...
class TCPDF {
	...
	/**
	 * Document keyworder_ids.
	 * @protected
	 */
	protected $keyworder_ids = '';
	...
	/**
	 * The right-bottom (or left-bottom for RTL) corner X coorder_idinate of last inserted image.
	 * @since 2002-07-31
	 * @author Nicola Asuni
	 * @protected
	 */
	...
	/**
	 * The right-bottom corner Y coorder_idinate of last inserted image.
	 * @since 2002-07-31
	 * @author Nicola Asuni
	 * @protected
	 */
	...
	/**
	 * HTML PARSER: array of boolean values, true in case of order_ided list (OL), false otherwise.
	 * @protected
	 */
	protected $listorder_ided = array();
	...
	/**
	 * Symbol used for HTML unorder_ided list items.
	 * @protected
	...
	/**
	 * Boolean flag to indicate if the border_id of the cell sides that cross the page should be removed.
	 * @protected
	...
	/**
	 * Regular expression used to find blank characters (required for worder_id-wrapping).
	 * @protected
	 ...
	 * @since 4.8.000 (2009-09-07)
	 */
	protected $default_form_prop = array('lineWidth'=>1, 'border_idStyle'=>'solid', 'fillColor'=>array(255, 255, 255), 'strokeColor'=>array(128, 128, 128));
	...
	public function __construct($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false, $pdfa=false) {
		...
		if ($this->isunicode AND (@preg_match('/\pL/u', 'a') == 1)) {
			...
			// \p{Z}  : any separator
			// \p{Lo} : Unicode letter or ideograph that does not have lowercase and uppercase variants. Is used to chunk chinese worder_ids.
		...
		$this->default_form_prop = array('lineWidth'=>1, 'border_idStyle'=>'solid', 'fillColor'=>array(255, 255, 255), 'strokeColor'=>array(128, 128, 128));
	 ...
	 * <li>['MediaBox']['llx'] : lower-left x coorder_idinate</li>
	 * <li>['MediaBox']['lly'] : lower-left y coorder_idinate</li>
	 * <li>['MediaBox']['urx'] : upper-right x coorder_idinate</li>
	 * <li>['MediaBox']['ury'] : upper-right y coorder_idinate</li>
	 * <li>['CropBox'] : the visible region of default user space:</li>
	 * <li>['CropBox']['llx'] : lower-left x coorder_idinate</li>
	 * <li>['CropBox']['lly'] : lower-left y coorder_idinate</li>
	 * <li>['CropBox']['urx'] : upper-right x coorder_idinate</li>
	 * <li>['CropBox']['ury'] : upper-right y coorder_idinate</li>
	 * <li>['BleedBox'] : the region to which the contents of the page shall be clipped when output in a production environment:</li>
	 * <li>['BleedBox']['llx'] : lower-left x coorder_idinate</li>
	 * <li>['BleedBox']['lly'] : lower-left y coorder_idinate</li>
	 * <li>['BleedBox']['urx'] : upper-right x coorder_idinate</li>
	 * <li>['BleedBox']['ury'] : upper-right y coorder_idinate</li>
	 * <li>['TrimBox'] : the intended dimensions of the finished page after trimming:</li>
	 * <li>['TrimBox']['llx'] : lower-left x coorder_idinate</li>
	 * <li>['TrimBox']['lly'] : lower-left y coorder_idinate</li>
	 * <li>['TrimBox']['urx'] : upper-right x coorder_idinate</li>
	 * <li>['TrimBox']['ury'] : upper-right y coorder_idinate</li>
	 * <li>['ArtBox'] : the extent of the page's meaningful content:</li>
	 * <li>['ArtBox']['llx'] : lower-left x coorder_idinate</li>
	 * <li>['ArtBox']['lly'] : lower-left y coorder_idinate</li>
	 * <li>['ArtBox']['urx'] : upper-right x coorder_idinate</li>
	 * <li>['ArtBox']['ury'] : upper-right y coorder_idinate</li>
	...
	public function setPageOrientation($orientation, $autopagebreak=null, $bottommargin=null) {
		...
		if ((abs($this->pagedim[$this->page]['MediaBox']['urx'] - $this->hPt) < $this->feps) AND (abs($this->pagedim[$this->page]['MediaBox']['ury'] - $this->wPt) < $this->feps)){
			// swap X and Y coorder_idinates (change page orientation)
			$this->pagedim = TCPDF_STATIC::swapPageBoxCoorder_idinates($this->page, $this->pagedim);
		}
	...
	/**
	 * Set regular expression to detect withespaces or worder_id separators.
	 ...
	 * <ul><li>$this->pagedim[$this->page]['w'] = page width in points</li><li>$this->pagedim[$this->page]['h'] = height in points</li><li>$this->pagedim[$this->page]['wk'] = page width in user units</li><li>$this->pagedim[$this->page]['hk'] = page height in user units</li><li>$this->pagedim[$this->page]['tm'] = top margin</li><li>$this->pagedim[$this->page]['bm'] = bottom margin</li><li>$this->pagedim[$this->page]['lm'] = left margin</li><li>$this->pagedim[$this->page]['rm'] = right margin</li><li>$this->pagedim[$this->page]['pb'] = auto page break</li><li>$this->pagedim[$this->page]['or'] = page orientation</li><li>$this->pagedim[$this->page]['olm'] = original left margin</li><li>$this->pagedim[$this->page]['orm'] = original right margin</li><li>$this->pagedim[$this->page]['Rotate'] = The number of degrees by which the page shall be rotated clockwise when displayed or printed. The value shall be a multiple of 90.</li><li>$this->pagedim[$this->page]['PZ'] = The page's preferred zoom (magnification) factor.</li><li>$this->pagedim[$this->page]['trans'] : the style and duration of the visual transition to use when moving from another page to the given page during a presentation<ul><li>$this->pagedim[$this->page]['trans']['Dur'] = The page's display duration (also called its advance timing): the maximum length of time, in seconds, that the page shall be displayed during presentations before the viewer application shall automatically advance to the next page.</li><li>$this->pagedim[$this->page]['trans']['S'] = transition style : Split, Blinds, Box, Wipe, Dissolve, Glitter, R, Fly, Push, Cover, Uncover, Fade</li><li>$this->pagedim[$this->page]['trans']['D'] = The duration of the transition effect, in seconds.</li><li>$this->pagedim[$this->page]['trans']['Dm'] = (Split and Blinds transition styles only) The dimension in which the specified transition effect shall occur: H = Horizontal, V = Vertical. Default value: H.</li><li>$this->pagedim[$this->page]['trans']['M'] = (Split, Box and Fly transition styles only) The direction of motion for the specified transition effect: I = Inward from the edges of the page, O = Outward from the center of the pageDefault value: I.</li><li>$this->pagedim[$this->page]['trans']['Di'] = (Wipe, Glitter, Fly, Cover, Uncover and Push transition styles only) The direction in which the specified transition effect shall moves, expressed in degrees counterclockwise starting from a left-to-right direction. If the value is a number, it shall be one of: 0 = Left to right, 90 = Bottom to top (Wipe only), 180 = Right to left (Wipe only), 270 = Top to bottom, 315 = Top-left to bottom-right (Glitter only). If the value is a name, it shall be None, which is relevant only for the Fly transition when the value of SS is not 1.0. Default value: 0.</li><li>$this->pagedim[$this->page]['trans']['SS'] = (Fly transition style only) The starting or ending scale at which the changes shall be drawn. If M specifies an inward transition, the scale of the changes drawn shall progress from SS to 1.0 over the course of the transition. If M specifies an outward transition, the scale of the changes drawn shall progress from 1.0 to SS over the course of the transition. Default: 1.0. </li><li>$this->pagedim[$this->page]['trans']['B'] = (Fly transition style only) If true, the area that shall be flown in is rectangular and opaque. Default: false.</li></ul></li><li>$this->pagedim[$this->page]['MediaBox'] : the boundaries of the physical medium on which the page shall be displayed or printed<ul><li>$this->pagedim[$this->page]['MediaBox']['llx'] = lower-left x coorder_idinate in points</li><li>$this->pagedim[$this->page]['MediaBox']['lly'] = lower-left y coorder_idinate in points</li><li>$this->pagedim[$this->page]['MediaBox']['urx'] = upper-right x coorder_idinate in points</li><li>$this->pagedim[$this->page]['MediaBox']['ury'] = upper-right y coorder_idinate in points</li></ul></li><li>$this->pagedim[$this->page]['CropBox'] : the visible region of default user space<ul><li>$this->pagedim[$this->page]['CropBox']['llx'] = lower-left x coorder_idinate in points</li><li>$this->pagedim[$this->page]['CropBox']['lly'] = lower-left y coorder_idinate in points</li><li>$this->pagedim[$this->page]['CropBox']['urx'] = upper-right x coorder_idinate in points</li><li>$this->pagedim[$this->page]['CropBox']['ury'] = upper-right y coorder_idinate in points</li></ul></li><li>$this->pagedim[$this->page]['BleedBox'] : the region to which the contents of the page shall be clipped when output in a production environment<ul><li>$this->pagedim[$this->page]['BleedBox']['llx'] = lower-left x coorder_idinate in points</li><li>$this->pagedim[$this->page]['BleedBox']['lly'] = lower-left y coorder_idinate in points</li><li>$this->pagedim[$this->page]['BleedBox']['urx'] = upper-right x coorder_idinate in points</li><li>$this->pagedim[$this->page]['BleedBox']['ury'] = upper-right y coorder_idinate in points</li></ul></li><li>$this->pagedim[$this->page]['TrimBox'] : the intended dimensions of the finished page after trimming<ul><li>$this->pagedim[$this->page]['TrimBox']['llx'] = lower-left x coorder_idinate in points</li><li>$this->pagedim[$this->page]['TrimBox']['lly'] = lower-left y coorder_idinate in points</li><li>$this->pagedim[$this->page]['TrimBox']['urx'] = upper-right x coorder_idinate in points</li><li>$this->pagedim[$this->page]['TrimBox']['ury'] = upper-right y coorder_idinate in points</li></ul></li><li>$this->pagedim[$this->page]['ArtBox'] : the extent of the page's meaningful content<ul><li>$this->pagedim[$this->page]['ArtBox']['llx'] = lower-left x coorder_idinate in points</li><li>$this->pagedim[$this->page]['ArtBox']['lly'] = lower-left y coorder_idinate in points</li><li>$this->pagedim[$this->page]['ArtBox']['urx'] = upper-right x coorder_idinate in points</li><li>$this->pagedim[$this->page]['ArtBox']['ury'] = upper-right y coorder_idinate in points</li></ul></li></ul>
	 ...
	 * @param string|array|int|bool $brd Indicates if border_ids must be drawn around the cell. The value can be a number:<ul><li>0: no border_id (default)</li><li>1: frame</li></ul> or a string containing some or all of the following characters (in any order_id):<ul><li>L: left</li><li>T: top</li><li>R: right</li><li>B: bottom</li></ul> or an array of line styles for each border_id group - for example: array('LTRB' => array('width' => 2, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => array(0, 0, 0)))
	...
	protected function adjustCellPadding($brd=0) {
		...
		// process border_ids
		foreach ($brd as $border_id => $style) {
	 ...
	 * @public
	 * @since 1.2
	 * @see SetAuthor(), SetCreator(), SetKeyworder_ids(), SetSubject()
	 */
	 ...
	 * @public
	 * @since 1.2
	 * @see SetAuthor(), SetCreator(), SetKeyworder_ids(), SetTitle()
	 */
	 ...
	 * @public
	 * @since 1.2
	 * @see SetCreator(), SetKeyworder_ids(), SetSubject(), SetTitle()
	 */
	...
	/**
	 * Associates keyworder_ids with the document, generally in the form 'keyworder_id1 keyworder_id2 ...'.
	 * @param string $keyworder_ids The list of keyworder_ids.
	 * @public
	 * @since 1.2
	 * @see SetAuthor(), SetCreator(), SetSubject(), SetTitle()
	 */
	public function setKeyworder_ids($keyworder_ids) {
		$this->keyworder_ids = $keyworder_ids;
	}
	 ...
	 * @public
	 * @since 1.2
	 * @see SetAuthor(), SetKeyworder_ids(), SetSubject(), SetTitle()
	 */
	...
	/**
	 * Adds a new page to the document. If a page is already present, the Footer() method is called first to output the footer (if enabled). Then the page is added, the current position set to the top-left corner accorder_iding to the left and top margins (or top-right if in RTL mode), and Header() is called to display the header (if enabled).
	 * The origin of the coorder_idinate system is at the top-left corner (or top-right for RTL) and increasing order_idinates go downwards.
	 ...
	 * The origin of the coorder_idinate system is at the top-left corner and increasing order_idinates go downwards.
	 * @param string $orientation page orientation. Possible values are (case insensitive):<ul><li>P or PORTRAIT (default)</li><li>L or LANDSCAPE</li></ul>
	 * @param mixed $format The format used for pages. It can be either: one of the string values specified at getPageSizeFromFormat() or an array of parameters specified at setPageFormat().
	...
	/**
	 * Set start-writing mark on current page stream used to put border_ids and fills.
	 * Border_ids and fills are always created after content and inserted on the position marked by this method.
	...
	public function setPageMark() {
		...
		$this->border_idmrk[$this->page] = $this->intmrk[$this->page];
		$this->setContentMark();
	}
	...
	/**
	 * Return the right-bottom (or left-bottom for RTL) corner X coorder_idinate of last inserted image
	 * @return float
	 * @public
	 */
	...
	/**
	 * Return the right-bottom (or left-bottom for RTL) corner Y coorder_idinate of last inserted image
	 * @return float
	 * @public
	 */
	...
	/**
	 * Defines the spot color used for all drawing operations (lines, rectangles and cell border_ids).
	 * @param string $name Name of the spot color.
	 * @param float $tint Intensity of the color (from 0 to 100 ; 100 = full intensity by default).
	 * @public
	 * @since 4.0.024 (2008-09-12)
	...
	/**
	 * Defines the color used for all drawing operations (lines, rectangles and cell border_ids).
	 * It can be expressed in RGB, CMYK or GRAY SCALE components.
	 * The method can be called before the first page is created and the value is retained from page to page.
	...
	/**
	 * Defines the color used for all drawing operations (lines, rectangles and cell border_ids). It can be expressed in RGB components or gray scale. The method can be called before the first page is created and the value is retained from page to page.
	 * @param float $col1 GRAY level for single color, or Red color for RGB (0-255), or CYAN color for CMYK (0-100).
	 * @param float $col2 GREEN color for RGB (0-255), or MAGENTA color for CMYK (0-100).
	 * @param float $col3 BLUE color for RGB (0-255), or YELLOW color for CMYK (0-100).
	 * @param float $col4 KEY (BLACK) color for CMYK (0-100).
	 ...
	 * @param mixed $subset if true embedd only a subset of the font (stores only the information related to the used characters); if false embedd full font; if 'default' uses the default value set using setFontSubsetting(). This option is valid only for TrueTypeUnicode fonts. If you want to enable users to change the document, set this parameter to false. If you subset the font, the person who receives your PDF would need to have your same font in order_id to make changes to your PDF. The file size of the PDF would also be smaller because you are embedding only part of a font.
	 * @public
	 * @since 1.5
	...
	public function AddFont($family, $style='', $fontfile='', $subset='default') {
		...
		if (!isset($cidinfo) OR TCPDF_STATIC::empty_string($cidinfo)) {
			$cidinfo = array('Registry'=>'Adobe', 'order_iding'=>'Identity', 'Supplement'=>0);
	 ...
	 * @param float $y order_idinate of target position; -1 indicates the current position. The default value is 0 (top of page)
	 ...
	 * @param float $y order_idinate of the upper-left corner of the rectangle
	...
	public function Annotation($x, $y, $w, $h, $text, $opt=array('Subtype'=>'Text'), $spaces=0) {
		...
		// recalculate coorder_idinates to account for graphic transformations
			...
			for ($i=$this->transfmatrix_key; $i > 0; --$i) {
				...
				for ($j=$maxid; $j >= 0; --$j) {
					...
					if (isset($ctm['a'])) {
						...
						// new coorder_idinates (rectangle area)
						$x = min($x1, $x2, $x3, $x4);
						$y = max($y1, $y2, $y3, $y4);
	 ...
	 * @param float $y order_idinate of the cell origin
	 ...
	 * @param mixed $border_id Indicates if border_ids must be drawn around the cell. The value can be a number:<ul><li>0: no border_id (default)</li><li>1: frame</li></ul> or a string containing some or all of the following characters (in any order_id):<ul><li>L: left</li><li>T: top</li><li>R: right</li><li>B: bottom</li></ul> or an array of line styles for each border_id group - for example: array('LTRB' => array('width' => 2, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => array(0, 0, 0)))
	 ...
	 */
	public function Text($x, $y, $txt, $fstroke=0, $fclip=false, $ffill=true, $border_id=0, $ln=0, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M', $rtloff=false) {
		$textrendermode = $this->textrendermode;
		$textstrokewidth = $this->textstrokewidth;
		$this->setTextRenderingMode($fstroke, $ffill, $fclip);
		$this->setXY($x, $y, $rtloff);
		$this->Cell(0, 0, $txt, $border_id, $ln, $align, $fill, $link, $stretch, $ignore_min_height, $calign, $valign);
	 ...
	 * The default implementation returns a value accorder_iding to the mode selected by SetAutoPageBreak().<br />
	...
	/**
	 * Prints a cell (rectangular area) with optional border_ids, background color and character string. The upper-left corner of the cell corresponds to the current position. The text can be aligned or centered. After the call, the current position moves to the right or to the next line. It is possible to put a link on the text.<br />
	 ...
	 */
	public function Cell($w, $h=0, $txt='', $border_id=0, $ln=0, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M') {
		$prev_cell_margin = $this->cell_margin;
		$prev_cell_padding = $this->cell_padding;
		$this->adjustCellPadding($border_id);
		...
		if ($this->txtshadow['enabled']) {
			...
			if ($this->state == 2) {
				$this->_out($this->getCellCode($w, $h, $txt, $border_id, $ln, $align, $fill, $link, $stretch, true, $calign, $valign));
			}
			//restore data
			$this->x = $x;
			$this->y = $y;
	...
	/**
	 * Returns the PDF string code to print a cell (rectangular area) with optional border_ids, background color and character string. The upper-left corner of the cell corresponds to the current position. The text can be aligned or centered. After the call, the current position moves to the right or to the next line. It is possible to put a link on the text.<br />
	 * If automatic page breaking is enabled and the cell goes beyond the limit, a page break is done before outputting.
	 * @param float $w Cell width. If 0, the cell extends up to the right margin.
	 * @param float $h Cell height. Default value: 0.
	 * @param string $txt String to print. Default value: empty string.
	 ...
	 * @see Cell()
	 */
	protected function getCellCode($w, $h=0, $txt='', $border_id=0, $ln=0, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M') {
		...
		$s = '';
		// fill and border_ids
		if (is_string($border_id) AND (strlen($border_id) == 4)) {
			// full border_id
			$border_id = 1;
		}
		if ($fill OR ($border_id == 1)) {
			if ($fill) {
				$op = ($border_id == 1) ? 'B' : 'f';
			} else {
				$op = 'S';
		...
		// draw border_ids
		$s .= $this->getCellBorder_id($x, $y, $w, $h, $border_id);
		if ($txt != '') {
			$txt2 = $txt;
			if ($this->isunicode) {
			...
			if (($align == 'J') AND ($ns > 0)) {
				if ($this->isUnicodeFont()) {
					...
					if ($this->font_stretching != 100) {
						// worder_id spacing is affected by stretching
						$spacewidth /= ($this->font_stretching / 100);
					}
					// set worder_id position to be used with TJ operator
					...
					if ($this->font_stretching != 100) {
						// worder_id spacing (Tw) is affected by stretching
						$spacewidth /= ($this->font_stretching / 100);
					}
					// set worder_id spacing
	...
	/**
	 * Returns the code to draw the cell border_id
	 * @param float $x X coorder_idinate.
	 * @param float $y Y coorder_idinate.
	 * @param float $w Cell width.
	 * @param float $h Cell height.
	 * @param string|array|int $brd Indicates if border_ids must be drawn around the cell. The value can be a number:<ul><li>0: no border_id (default)</li><li>1: frame</li></ul> or a string containing some or all of the following characters (in any order_id):<ul><li>L: left</li><li>T: top</li><li>R: right</li><li>B: bottom</li></ul> or an array of line styles for each border_id group - for example: array('LTRB' => array('width' => 2, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => array(0, 0, 0)))
	 * @return string containing cell border_id code
	 * @protected
	 * @see SetLineStyle()
	 * @since 5.7.000 (2010-08-02)
	 */
	protected function getCellBorder_id($x, $y, $w, $h, $brd) {
		...
		// calculate coorder_idinates for border_id
		$k = $this->k;
		if ($this->rtl) {
			$xeL = ($x - $w) * $k;
			$xeR = $x * $k;
		...
		foreach ($brd as $border_id => $style) {
			...
			// draw border_ids by case
			if (strlen($border_id) == 4) {
				$s .= sprintf('%F %F %F %F re S ', $xT, $yT, ($w * $k), (-$h * $k));
			} elseif (strlen($border_id) == 3) {
				if (strpos($border_id,'B') === false) { // LTR
					$s .= sprintf('%F %F m ', $xL, $yL);
					$s .= sprintf('%F %F l ', $xT, $yT);
					$s .= sprintf('%F %F l ', $xR, $yR);
					$s .= sprintf('%F %F l ', $xB, $yB);
					$s .= 'S ';
				} elseif (strpos($border_id,'L') === false) { // TRB
					$s .= sprintf('%F %F m ', $xT, $yT);
					$s .= sprintf('%F %F l ', $xR, $yR);
					$s .= sprintf('%F %F l ', $xB, $yB);
					$s .= sprintf('%F %F l ', $xL, $yL);
					$s .= 'S ';
				} elseif (strpos($border_id,'T') === false) { // RBL
					$s .= sprintf('%F %F m ', $xR, $yR);
					$s .= sprintf('%F %F l ', $xB, $yB);
					$s .= sprintf('%F %F l ', $xL, $yL);
					$s .= sprintf('%F %F l ', $xT, $yT);
					$s .= 'S ';
				} elseif (strpos($border_id,'R') === false) { // BLT
			...
			} elseif (strlen($border_id) == 2) {
				if ((strpos($border_id,'L') !== false) AND (strpos($border_id,'T') !== false)) { // LT
					$s .= sprintf('%F %F m ', $xL, $yL);
					$s .= sprintf('%F %F l ', $xT, $yT);
					$s .= sprintf('%F %F l ', $xR, $yR);
					$s .= 'S ';
				} elseif ((strpos($border_id,'T') !== false) AND (strpos($border_id,'R') !== false)) { // TR
					$s .= sprintf('%F %F m ', $xT, $yT);
					$s .= sprintf('%F %F l ', $xR, $yR);
					$s .= sprintf('%F %F l ', $xB, $yB);
					$s .= 'S ';
				} elseif ((strpos($border_id,'R') !== false) AND (strpos($border_id,'B') !== false)) { // RB
					$s .= sprintf('%F %F m ', $xR, $yR);
					$s .= sprintf('%F %F l ', $xB, $yB);
					$s .= sprintf('%F %F l ', $xL, $yL);
					$s .= 'S ';
				} elseif ((strpos($border_id,'B') !== false) AND (strpos($border_id,'L') !== false)) { // BL
					$s .= sprintf('%F %F m ', $xB, $yB);
					$s .= sprintf('%F %F l ', $xL, $yL);
					$s .= sprintf('%F %F l ', $xT, $yT);
					$s .= 'S ';
				} elseif ((strpos($border_id,'L') !== false) AND (strpos($border_id,'R') !== false)) { // LR
					$s .= sprintf('%F %F m ', $xL, $yL);
					$s .= sprintf('%F %F l ', $xT, $yT);
					$s .= 'S ';
					$s .= sprintf('%F %F m ', $xR, $yR);
					$s .= sprintf('%F %F l ', $xB, $yB);
					$s .= 'S ';
				} elseif ((strpos($border_id,'T') !== false) AND (strpos($border_id,'B') !== false)) { // TB
					$s .= sprintf('%F %F m ', $xT, $yT);
					$s .= sprintf('%F %F l ', $xR, $yR);
					$s .= 'S ';
					$s .= sprintf('%F %F m ', $xB, $yB);
			...
			} else { // strlen($border_id) == 1
				if (strpos($border_id,'L') !== false) { // L
					$s .= sprintf('%F %F m ', $xL, $yL);
					$s .= sprintf('%F %F l ', $xT, $yT);
					$s .= 'S ';
				} elseif (strpos($border_id,'T') !== false) { // T
					$s .= sprintf('%F %F m ', $xT, $yT);
					$s .= sprintf('%F %F l ', $xR, $yR);
					$s .= 'S ';
				} elseif (strpos($border_id,'R') !== false) { // R
					$s .= sprintf('%F %F m ', $xR, $yR);
					$s .= sprintf('%F %F l ', $xB, $yB);
					$s .= 'S ';
				} elseif (strpos($border_id,'B') !== false) { // B
					$s .= sprintf('%F %F m ', $xB, $yB);
					$s .= sprintf('%F %F l ', $xL, $yL);
					$s .= 'S ';
				}
			...
			if (is_array($style) AND !empty($style)) {
				// reset border_id style to previous value
	 ...
	 * They can be automatic (as soon as the text reaches the right border_id of the cell) or explicit (via the \n character). As many cells as necessary are output, one below the other.<br />
	 ...
	 */
	public function MultiCell($w, $h, $txt, $border_id=0, $align='J', $fill=false, $ln=1, $x=null, $y=null, $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0, $valign='T', $fitcell=false) {
		$prev_cell_margin = $this->cell_margin;
		$prev_cell_padding = $this->cell_padding;
		...
		if (!TCPDF_STATIC::empty_string($y)) {
			$this->setY($y); // set y in order_id to convert negative y values to positive ones
		}
		$y = $this->GetY();
		$resth = 0;
		...
		if ($ishtml) { // ******* Write HTML text
			...
			if ($maxh > 0) {
				// get text height
				$text_height = $this->getStringHeight($w, $txt, $reseth, $autopadding, $mc_padding, $border_id);
		...
		// get border_id modes
		$border_id_start = TCPDF_STATIC::getBorder_idMode($border_id, $position='start', $this->opencell);
		$border_id_end = TCPDF_STATIC::getBorder_idMode($border_id, $position='end', $this->opencell);
		$border_id_middle = TCPDF_STATIC::getBorder_idMode($border_id, $position='middle', $this->opencell);
		// design border_ids around HTML cells.
		for ($page = $startpage; $page <= $endpage; ++$page) { // for each page
			...
			if ($startpage == $endpage) {
				...
				for ($column = $startcolumn; $column <= $endcolumn; ++$column) { // for each column
					...
					if ($startcolumn == $endcolumn) { // single column
						$cborder_id = $border_id;
						$h = max($h, ($currentY - $oy));
						$this->y = $oy;
					} elseif ($column == $startcolumn) { // first column
						$cborder_id = $border_id_start;
						$this->y = $oy;
						$h = $this->h - $this->y - $this->bMargin;
					} elseif ($column == $endcolumn) { // end column
						$cborder_id = $border_id_end;
						$h = $currentY - $this->y;
						if ($resth > $h) {
							$h = $resth;
						}
					} else { // middle column
						$cborder_id = $border_id_middle;
						$h = $this->h - $this->y - $this->bMargin;
						$resth -= $h;
					}
					$ccode .= $this->getCellCode($w, $h, '', $cborder_id, 1, '', $fill, '', 0, true)."\n";
				} // end for each column
			...
			if ($cborder_id OR $fill) {
				$offsetlen = strlen($ccode);
				// draw border_id and fill
				if ($this->inxobj) {
					// we are inside an XObject template
	 ...
	 * @since 4.5.011
	 */
	public function getNumLines($txt, $w=0, $reseth=false, $autopadding=true, $cellpadding=null, $border_id=0) {
		if ($txt === NULL) {
			return 0;
		}
		if ($txt === '') {
	 ...
	 *  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	 *  $pdf->MultiCell($w=0, $h=0, $txt, $border_id=1, $align='L', $fill=false, $ln=1, $x=null, $y=null, $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0);
	 *  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	 *  // get the new Y
	 *  $end_y = $pdf->GetY();
	 ...
	 * @public
	 */
	public function getStringHeight($w, $txt, $reseth=false, $autopadding=true, $cellpadding=null, $border_id=0) {
		// adjust internal padding
		$prev_cell_padding = $this->cell_padding;
		$prev_lasth = $this->lasth;
		if (is_array($cellpadding)) {
		...
		$this->adjustCellPadding($border_id);
		$lines = $this->getNumLines($txt, $w, $reseth, $autopadding, $cellpadding, $border_id);
				...
				if (($l > $wmax) OR (($c == 173) AND (($l + $tmp_shy_replacement_width) >= $wmax))) {
					...
					if ($sep == -1) {
						...
						if (($this->rtl AND ($this->x <= ($this->w - $this->rMargin - $this->cell_padding['R'] - $margin['R'] - $chrwidth)))
							...
							// truncate the worder_id because do not fit on column
							$tmpstr = TCPDF_FONTS::UniArrSubString($uchars, $j, $i);
							if ($firstline) {
								$startx = $this->x;
								$tmparr = array_slice($chars, $j, ($i - $j));
						...
						if (isset($nextstr[0]) AND ($this->GetStringWidth($nextstr[0]) > $pw)) {
							// truncate the worder_id because do not fit on a full page width
							$tmpstr = TCPDF_FONTS::UniArrSubString($uchars, $j, $i);
							if ($firstline) {
								$startx = $this->x;
								$tmparr = array_slice($chars, $j, ($i - $j));
	 ...
	 * <li>one explicit dimension, the other being calculated automatically in order_id to keep the original proportions</li>
	 ...
	 * @param float|null $y order_idinate of the upper-left corner (LTR) or upper-right corner (RTL).
	 ...
	 * @since 1.1
	 */
	public function Image($file, $x=null, $y=null, $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border_id=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		if ($this->state != 2) {
			return false;
		}
		if (TCPDF_STATIC::empty_string($x)) {
		...
		if (!empty($border_id)) {
			...
			$this->y = $y;
			$this->Cell($w, $h, '', $border_id, 0, '', 0, '', 0, true);
			$this->x = $bx;
			$this->y = $by;
		}
		if ($link) {
	 ...
	 * @param float $y order_idinate of the upper-left corner.
	 ...
	 * The current abscissa goes back to the left margin and the order_idinate increases by the amount passed in parameter.
	 * @param float|null $h The height of the break. By default, the value equals the height of the last printed cell.
	 * @param boolean $cell if true add the current left (or right o for RTL) padding to the X coorder_idinate
	 * @public
	 * @since 1.0
	 * @see Cell()
	 */
	 ...
	 * The value is relative to the left border_id for LTR languages and to the right border_id for RTL languages.
	 * @return float
	 * @public
	 * @since 1.2
	 * @see SetX(), GetY(), SetY()
	...
	/**
	 * Returns the order_idinate of the current position.
	 * @return float
	 * @public
	 * @since 1.0
	 * @see SetY(), GetX(), SetX()
	...
	/**
	 * Moves the current abscissa back to the left margin and sets the order_idinate.
	 * If the passed value is negative, it is relative to the bottom of the page.
	 * @param float $y The value of the order_idinate in user units.
	...
	/**
	 * Defines the abscissa and order_idinate of the current position.
	 * If the passed values are negative, they are relative respectively to the right and bottom of the page.
	 * @param float $x The value of the abscissa.
	 * @param float $y The value of the order_idinate.
	 * @param boolean $rtloff if true always uses the page top-left corner as origin of axis.
	 * @public
	 * @since 1.2
	 * @see SetX(), SetY()
	...
	/**
	 * Set the absolute X coorder_idinate of the current pointer.
	 * @param float $x The value of the abscissa in user units.
	 * @public
	 * @since 5.9.186 (2012-09-13)
	...
	/**
	 * Set the absolute Y coorder_idinate of the current pointer.
	 * @param float $y (float) The value of the order_idinate in user units.
	 * @public
	 * @since 5.9.186 (2012-09-13)
	 * @see setAbsX(), setAbsY(), SetAbsXY()
	 */
	...
	/**
	 * Set the absolute X and Y coorder_idinates of the current pointer.
	 * @param float $x The value of the abscissa in user units.
	 * @param float $y (float) The value of the order_idinate in user units.
	 * @public
	 * @since 5.9.186 (2012-09-13)
	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		if ($this->sign) {
			...
			if (empty($this->signature_data['extracerts'])) {
				openssl_pkcs7_sign($tempdoc, $tempsign, $this->signature_data['signcert'], array($this->signature_data['privkey'], $this->signature_data['passworder_id']), array(), PKCS7_BINARY | PKCS7_DETACHED);
			} else {
				openssl_pkcs7_sign($tempdoc, $tempsign, $this->signature_data['signcert'], array($this->signature_data['privkey'], $this->signature_data['passworder_id']), array(), PKCS7_BINARY | PKCS7_DETACHED, $this->signature_data['extracerts']);
			}
			// read signature
	...
	protected function getInternalPageNumberAliases($a= '') {
		$alias = array();
		// build array of Unicode + ASCII variants (the order_id is important)
				...
				foreach ($this->PageAnnots[$n] as $key => $pl) {
					...
					if (isset($pl['opt']['bs']) AND (is_array($pl['opt']['bs']))) {
						...
						if (isset($pl['opt']['bs']['d']) AND (is_array($pl['opt']['bs']['d']))) {
							$annots .= ' /D [';
							foreach ($pl['opt']['bs']['d'] as $corder_id) {
								$annots .= ' '.intval($corder_id);
							}
							$annots .= ']';
						}
						$annots .= ' >>';
						...
						$annots .= ' /Border_id [';
						if (isset($pl['opt']['border_id']) AND (count($pl['opt']['border_id']) >= 3)) {
							$annots .= intval($pl['opt']['border_id'][0]).' ';
							$annots .= intval($pl['opt']['border_id'][1]).' ';
							$annots .= intval($pl['opt']['border_id'][2]);
							if (isset($pl['opt']['border_id'][3]) AND is_array($pl['opt']['border_id'][3])) {
								$annots .= ' [';
								foreach ($pl['opt']['border_id'][3] as $dash) {
									$annots .= intval($dash).' ';
								}
								$annots .= ']';
							}
		...
		foreach ($this->FontFiles as $file => $info) {
			...
			if (!TCPDF_STATIC::empty_string($fontfile)) {
				...
				if ((!$compressed) AND (isset($info['length2']))) {
					$header = (order_id($font[0]) == 128);
					if ($header) {
						// strip first binary header
						$font = substr($font, 6);
					}
					if ($header AND (order_id($font[$info['length1']]) == 128)) {
	...
	protected function _puttruetypeunicode($font) {
		...
		$cidinfo .= ' /order_iding '.$this->_datastring($font['cidinfo']['order_iding'], $oid);
	...
	protected function _putinfo() {
		...
		if (!TCPDF_STATIC::empty_string($this->keyworder_ids)) {
			// Keyworder_ids associated with the document.
			$out .= ' /Keyworder_ids '.$this->_textstring($this->keyworder_ids, $oid);
		}
	...
	protected function _putXMP() {
		...
		$xmp .= "\t\t\t".'<dc:subject>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Bag>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li>'.TCPDF_STATIC::_escapeXML($this->keyworder_ids).'</rdf:li>'."\n";
		$xmp .= "\t\t\t\t".'</rdf:Bag>'."\n";
		$xmp .= "\t\t\t".'</dc:subject>'."\n";
		$xmp .= "\t\t".'</rdf:Description>'."\n";
		...
		$xmp .= "\t\t\t".'<pdf:Keyworder_ids>'.TCPDF_STATIC::_escapeXML($this->keyworder_ids).'</pdf:Keyworder_ids>'."\n";
	...
	/**
	 * Underline text.
	 * @param int $x X coorder_idinate
	 * @param int $y Y coorder_idinate
	...
	protected function _Uvalue() {
		if ($this->encryptdata['mode'] == 0) { // RC4-40
			...
			for ($i = 1; $i <= 19; ++$i) {
				$ek = '';
				for ($j = 0; $j < $len; ++$j) {
					$ek .= chr(order_id($this->encryptdata['key'][$j]) ^ $i);
				}
			...
			return hash('sha256', $this->encryptdata['user_passworder_id'].$this->encryptdata['UVS'], true).$this->encryptdata['UVS'].$this->encryptdata['UKS'];
		}
	}
	...
	protected function _UEvalue() {
		$hashkey = hash('sha256', $this->encryptdata['user_passworder_id'].$this->encryptdata['UKS'], true);
	...
	protected function _Ovalue() {
		if ($this->encryptdata['mode'] < 3) { // RC4-40, RC4-128, AES-128
			$tmp = TCPDF_STATIC::_md5_16($this->encryptdata['owner_passworder_id']);
			...
			$enc = TCPDF_STATIC::_RC4($owner_key, $this->encryptdata['user_passworder_id'], $this->last_enc_key, $this->last_enc_key_c);
			if ($this->encryptdata['mode'] > 0) {
				$len = strlen($owner_key);
				for ($i = 1; $i <= 19; ++$i) {
					$ek = '';
					for ($j = 0; $j < $len; ++$j) {
						$ek .= chr(order_id($owner_key[$j]) ^ $i);
					}
					$enc = TCPDF_STATIC::_RC4($ek, $enc, $this->last_enc_key, $this->last_enc_key_c);
				}
			}
			...
			return hash('sha256', $this->encryptdata['owner_passworder_id'].$this->encryptdata['OVS'].$this->encryptdata['U'], true).$this->encryptdata['OVS'].$this->encryptdata['OKS'];
		}
	}
	...
	protected function _OEvalue() {
		$hashkey = hash('sha256', $this->encryptdata['owner_passworder_id'].$this->encryptdata['OKS'].$this->encryptdata['U'], true);
		return TCPDF_STATIC::_AESnopad($hashkey, $this->encryptdata['key']);
	}

	/**
	 * Convert passworder_id for AES-256 encryption mode
	 * @param string $passworder_id passworder_id
	 * @return string passworder_id
	 * @protected
	 * @since 5.9.006 (2010-10-19)
	 * @author Nicola Asuni
	 */
	protected function _fixAES256Passworder_id($passworder_id) {
		$psw = ''; // passworder_id to be returned
		$psw_array = TCPDF_FONTS::utf8Bidi(TCPDF_FONTS::UTF8StringToArray($passworder_id, $this->isunicode, $this->CurrentFont), $passworder_id, $this->rtl, $this->isunicode, $this->CurrentFont);
	...
	protected function _generateencryptionkey() {
		...
		if (!$this->encryptdata['pubkey']) { // standard mode
			if ($this->encryptdata['mode'] == 3) { // AES-256
				...
				// truncate passworder_ids
				$this->encryptdata['user_passworder_id'] = $this->_fixAES256Passworder_id($this->encryptdata['user_passworder_id']);
				$this->encryptdata['owner_passworder_id'] = $this->_fixAES256Passworder_id($this->encryptdata['owner_passworder_id']);
				// Compute U value
				...
				// Pad passworder_ids
				$this->encryptdata['user_passworder_id'] = substr($this->encryptdata['user_passworder_id'].TCPDF_STATIC::$enc_padding, 0, 32);
				$this->encryptdata['owner_passworder_id'] = substr($this->encryptdata['owner_passworder_id'].TCPDF_STATIC::$enc_padding, 0, 32);
				// Compute O value
				$this->encryptdata['O'] = $this->_Ovalue();
				// get default permissions (reverse byte order_id)
				$permissions = TCPDF_STATIC::getEncPermissionsString($this->encryptdata['protection']);
				// Compute encryption key
				$tmp = TCPDF_STATIC::_md5_16($this->encryptdata['user_passworder_id'].$this->encryptdata['O'].$permissions.$this->encryptdata['fileid']);
				if ($this->encryptdata['mode'] > 0) {
					for ($i = 0; $i < 50; ++$i) {
			...
			foreach ($this->encryptdata['pubkeys'] as $pubkey) {
				...
				// The bytes of each item in the Recipients array of PKCS#7 objects in the order_id in which they appear in the array
	 ...
	 * If you don't set any passworder_id, the document will open as usual. If you set a user passworder_id, the PDF viewer will ask for it before displaying the document. The master passworder_id, if different from the user one, can be used to get full access.
	 ...
	 * @param string $user_pass user passworder_id. Empty by default.
	 * @param string|null $owner_pass owner passworder_id. If not specified, a random value is used.
	...
	public function setProtection($permissions=array('print', 'modify', 'copy', 'annot-forms', 'fill-forms', 'extract', 'assemble', 'print-high'), $user_pass='', $owner_pass=null, $mode=0, $pubkeys=null) {
		...
		if (($pubkeys !== null) AND (is_array($pubkeys))) {
			...
			// standard mode (passworder_id mode)
		...
		$this->encryptdata['user_passworder_id'] = $user_pass;
		$this->encryptdata['owner_passworder_id'] = $owner_pass;
	 ...
	 * @param int $y order_idinate of the scaling center. Default is current y position
	 * @public
	 * @since 2.1.000 (2008-01-07)
	 * @see StartTransform(), StopTransform()
	 */
	 ...
	 * @param float|null $y order_idinate of the scaling center. Default is current y position
	 * @public
	 * @since 2.1.000 (2008-01-07)
	 * @see StartTransform(), StopTransform()
	 */
	public function Scale($s_x, $s_y, $x=null, $y=null) {
		...
		$tm[4] = $x * (1 - $s_x);
		$tm[5] = $y * (1 - $s_y);
		//scale the coorder_idinate system
		$this->Transform($tm);
	}
	...
	/**
	 * Verical Mirroring.
	 * @param float|null $y order_idinate of the point. Default is current y position
	 * @public
	 * @since 2.1.000 (2008-01-07)
	 * @see StartTransform(), StopTransform()
	 */
	...
	public function Translate($t_x, $t_y) {
		...
		$tm[4] = $t_x * $this->k;
		$tm[5] = -$t_y * $this->k;
		//translate the coorder_idinate system
		$this->Transform($tm);
	}
	 ...
	 * @param float|null $y order_idinate of the rotation center. Default is current y position
	 * @public
	 * @since 2.1.000 (2008-01-07)
	 * @see StartTransform(), StopTransform()
	 */
	public function Rotate($angle, $x=null, $y=null) {
		...
		//rotate the coorder_idinate system around ($x,$y)
		$this->Transform($tm);
	}
	 ...
	 * @param float|null $y order_idinate of the skewing center. default is current y position
	 * @public
	 * @since 2.1.000 (2008-01-07)
	 * @see StartTransform(), StopTransform()
	 */
	...
	public function Skew($angle_x, $angle_y, $x=null, $y=null) {
		...
		$tm[4] = -$tm[2] * $y;
		$tm[5] = -$tm[1] * $x;
		//skew the coorder_idinate system
		$this->Transform($tm);
	}
	...
	/**
	 * Begin a new subpath by moving the current point to coorder_idinates (x, y), omitting any connecting line segment.
	 * @param float $x Abscissa of point.
	 * @param float $y order_idinate of point.
	 * @protected
	 * @since 2.1.000 (2008-01-08)
	 */
	 ...
	 * @param float $y order_idinate of end point.
	 * @protected
	 * @since 2.1.000 (2008-01-08)
	 */
	 ...
	 * @param float $y order_idinate of upper-left corner.
	 * @param float $w Width.
	 * @param float $h Height.
	 ...
	 * @param float $y1 order_idinate of control point 1.
	 * @param float $x2 Abscissa of control point 2.
	 * @param float $y2 order_idinate of control point 2.
	 * @param float $x3 Abscissa of end point.
	 * @param float $y3 order_idinate of end point.
	 * @protected
	 * @since 2.1.000 (2008-01-08)
	 */
	 ...
	 * @param float $y1 order_idinate of first point.
	 * @param float $x2 Abscissa of second point.
	 * @param float $y2 order_idinate of second point.
	 ...
	 * @param array $border_id_style Border_id style of rectangle. Array with keys among the following:
	 * <ul>
	 *	 <li>all: Line style of all border_ids. Array like for SetLineStyle().</li>
	 *	 <li>L, T, R, B or combinations: Line style of left, top, right or bottom border_id. Array like for SetLineStyle().</li>
	 * </ul>
	 * If a key is not present or is null, the correspondent border_id is not drawn. Default value: default line style (empty array).
	 ...
	 * @see SetLineStyle()
	 */
	public function Rect($x, $y, $w, $h, $style='', $border_id_style=array(), $fill_color=array()) {
		if ($this->state != 2) {
			return;
		}
		if (empty($style)) {
		...
		if (!empty($border_id_style)) {
			if (isset($border_id_style['all']) AND !empty($border_id_style['all'])) {
				//set global style for border_id
				$this->setLineStyle($border_id_style['all']);
				$border_id_style = array();
			} else {
		...
		if (!empty($border_id_style)) {
			$border_id_style2 = array();
			foreach ($border_id_style as $line => $value) {
				$length = strlen($line);
				for ($i = 0; $i < $length; ++$i) {
					$border_id_style2[$line[$i]] = $value;
				}
			}
			$border_id_style = $border_id_style2;
			if (isset($border_id_style['L']) AND $border_id_style['L']) {
				$this->Line($x, $y, $x, $y + $h, $border_id_style['L']);
			}
			if (isset($border_id_style['T']) AND $border_id_style['T']) {
				$this->Line($x, $y, $x + $w, $y, $border_id_style['T']);
			}
			if (isset($border_id_style['R']) AND $border_id_style['R']) {
				$this->Line($x + $w, $y, $x + $w, $y + $h, $border_id_style['R']);
			}
			if (isset($border_id_style['B']) AND $border_id_style['B']) {
				$this->Line($x, $y + $h, $x + $w, $y + $h, $border_id_style['B']);
			}
		}
	}
	 ...
	 * @param float $y0 order_idinate of start point.
	 * @param float $x1 Abscissa of control point 1.
	 * @param float $y1 order_idinate of control point 1.
	 * @param float $x2 Abscissa of control point 2.
	 * @param float $y2 order_idinate of control point 2.
	 ...
	 * @param float $y0 order_idinate of center point.
	 ...
	 * @param float $yc order_idinate of center point.
	 * @param float $rx Horizontal radius.
	 * @param float $ry Vertical radius (if ry = 0 then is a circle, see Circle()). Default value: 0.
	 ...
	 * @param boolean $pie if true do not mark the border_id point (used to draw pie sectors).
	 * @param integer $nc Number of curves used to draw a 90 degrees portion of ellipse.
	 ...
	 * @return array bounding box coorder_idinates (x min, y min, x max, y max)
	 * @author Nicola Asuni
	 * @protected
	 * @since 4.9.019 (2010-04-26)
	 */
	protected function _outellipticalarc($xc, $yc, $rx, $ry, $xang=0, $angs=0, $angf=360, $pie=false, $nc=2, $startpoint=true, $ccw=true, $svg=false) {
		...
		// center point in PDF coorder_idinates
		$x0 = $xc;
		$y0 = ($this->h - $yc);
		// starting angle
		$ang = $as;
		...
		for ($i = 1; $i <= $nc; ++$i) {
			...
			// get bounding box coorder_idinates
	...
	public function Polygon($p, $style='', $line_style=array(), $fill_color=array(), $closed=true) {
		...
		$nc = count($p); // number of coorder_idinates
	 ...
	 * @param string $round_corner Draws rounded corner or not. String with a 0 (not rounded i-corner) or 1 (rounded i-corner) in i-position. Positions are, in order_id and begin to 0: top right, bottom right, bottom left and top left. Default value: all rounded corner ("1111").
	 * @param string $style Style of rendering. See the getPathPaintOperator() function for more information.
	 * @param array $border_id_style Border_id style of rectangle. Array like for SetLineStyle(). Default value: default line style (empty array).
	 * @param array $fill_color Fill color. Format: array(GREY) or array(R,G,B) or array(C,M,Y,K) or array(C,M,Y,K,SpotColorName). Default value: default color (empty array).
	 * @public
	 * @since 2.1.000 (2008-01-08)
	 */
	public function RoundedRect($x, $y, $w, $h, $r, $round_corner='1111', $style='', $border_id_style=array(), $fill_color=array()) {
		$this->RoundedRectXY($x, $y, $w, $h, $r, $r, $round_corner, $style, $border_id_style, $fill_color);
	}
	 ...
	 */
	public function RoundedRectXY($x, $y, $w, $h, $rx, $ry, $round_corner='1111', $style='', $border_id_style=array(), $fill_color=array()) {
		if ($this->state != 2) {
			return;
		}
		if (($round_corner == '0000') OR (($rx == $ry) AND ($rx == 0))) {
			// Not rounded
			$this->Rect($x, $y, $w, $h, $style, $border_id_style, $fill_color);
			return;
		}
		// Rounded
		if (!(false === strpos($style, 'F')) AND is_array($fill_color)) {
		...
		if ($border_id_style) {
			$this->setLineStyle($border_id_style);
		}
		$MyArc = 4 / 3 * (sqrt(2) - 1);
	 ...
	 * @param float $y0 order_idinate of first point.
	 * @param float $x1 Abscissa of second point.
	 * @param float $y1 order_idinate of second point.
	 ...
	 * @param float|null $y order_idinate of the upper-left corner of the rectangle
	...
	public function Button($name, $w, $h, $caption, $action, $prop=array(), $opt=array(), $x=null, $y=null, $js=false) {
		...
		$this->Cell($w, $h, $caption, $border_id, 0, 'C', true, '', 1, false, 'T', 'M');
		$this->endTemplate();
		--$this->n;
		$popt['ap']['n'] .= $this->xobjects[$tmpid]['outdata'];
		unset($this->xobjects[$tmpid]);
	 ...
	 * @param string $private_key_passworder_id passworder_id
	 ...
	 * @since 4.6.005 (2009-04-24)
	 */
	public function setSignature($signing_cert='', $private_key='', $private_key_passworder_id='', $extracerts='', $cert_type=2, $info=array(), $approval='') {
		// to create self-signed signature: openssl req -x509 -nodes -days 365000 -newkey rsa:1024 -keyout tcpdf.crt -out tcpdf.crt
		// to export crt to p12: openssl pkcs12 -export -in tcpdf.crt -out tcpdf.p12
		...
		if (strlen($signing_cert) == 0) {
			$this->Error('Please provide a certificate file and passworder_id!');
		}
		...
		$this->signature_data['passworder_id'] = $private_key_passworder_id;
	...
	/**
	 * Get the array that defines the signature appearance (page and rectangle coorder_idinates).
	 * @param float $x Abscissa of the upper-left corner.
	 * @param float $y order_idinate of the upper-left corner.
	 * @param float $w Width of the signature area.
	 * @param float $h Height of the signature area.
	 * @param int $page option page number (if < 0 the current page is used).
	 * @param string $name Name of the signature.
	 * @return array Array defining page and rectangle coorder_idinates of signature appearance.
	 * @protected
	 * @author Nicola Asuni
	 * @since 5.9.101 (2011-07-06)
	 */
	 ...
	 * The trusted timestamping improve document security that means that no one should be able to change the document once it has been recorder_ided.
	 ...
	 * @param string $tsa_passworder_id Specifies the passworder_id for TSA authorization (optional)
	 ...
	 */
	public function setTimeStamp($tsa_host='', $tsa_username='', $tsa_passworder_id='', $tsa_cert='') {
		$this->tsa_data = array();
		...
		$this->tsa_data['tsa_passworder_id'] = $tsa_passworder_id;
	 ...
	 * @param string $bm blend mode, one of the following: Normal, Multiply, Screen, Overlay, Darken, Lighten, Colorder_idodge, ColorBurn, HardLight, SoftLight, Difference, Exclusion, Hue, Saturation, Color, Luminosity
	...
	public function setAlpha($stroking=1, $bm='Normal', $nonstroking=null, $ais=false) {
		...
		if (!in_array($bm, array('Normal', 'Multiply', 'Screen', 'Overlay', 'Darken', 'Lighten', 'Colorder_idodge', 'ColorBurn', 'HardLight', 'SoftLight', 'Difference', 'Exclusion', 'Hue', 'Saturation', 'Color', 'Luminosity'))) {
			$bm = 'Normal';
		}
		$ais = $ais ? true : false;
	 ...
	 * @param float $y order_idinate of the top left corner of the rectangle.
	 * @param float $w width of the rectangle.
	 * @param float $h height of the rectangle.
	...
	public function colorRegistrationBar($x, $y, $w, $h, $transition=true, $vertical=false, $colors='A,R,G,B,C,M,Y,K') {
		...
		// set bar measures
		if ($vertical) {
			$coorder_ids = array(0, 0, 0, 1);
			$wb = $w / $numbars; // bar width
			$hb = $h; // bar height
			$xd = $wb; // delta x
			$yd = 0; // delta y
		} else {
			$coorder_ids = array(1, 0, 0, 0);
			$wb = $w; // bar width
		...
		foreach ($bars as $col) {
			...
			if ($col != 'NONE') {
				if ($transition) {
					// color gradient
					$this->LinearGradient($xb, $yb, $wb, $hb, $col_a, $col_b, $coorder_ids);
				} else {
	 ...
	 * @param float $y order_idinate of the crop mark center.
	 ...
	 * @param float $y order_idinate of the registration mark center.
	 ...
	 * @param array $coorder_ids array of the form (x1, y1, x2, y2) which defines the gradient vector (see linear_gradient_coorder_ids.jpg). The default value is from left to right (x1=0, y1=0, x2=1, y2=0).
	 * @author Andreas W\FCrmser, Nicola Asuni
	 * @since 3.1.000 (2008-06-09)
	 * @public
	 */
	public function LinearGradient($x, $y, $w, $h, $col1=array(), $col2=array(), $coorder_ids=array(0,0,1,0)) {
		$this->Clip($x, $y, $w, $h);
		$this->Gradient(2, $coorder_ids, array(array('color' => $col1, 'offset' => 0, 'exponent' => 1), array('color' => $col2, 'offset' => 1, 'exponent' => 1)), array(), false);
	}
	 ...
	 * @param array $col1 first color (Grayscale, RGB or CMYK components).
	 * @param array $col2 second color (Grayscale, RGB or CMYK components).
	 * @param array $coorder_ids array of the form (fx, fy, cx, cy, r) where (fx, fy) is the starting point of the gradient with color1, (cx, cy) is the center of the circle with color2, and r is the radius of the circle (see radial_gradient_coorder_ids.jpg). (fx, fy) should be inside the circle, otherwise some areas will not be defined.
	 * @author Andreas W\FCrmser, Nicola Asuni
	 * @since 3.1.000 (2008-06-09)
	 * @public
	 */
	public function RadialGradient($x, $y, $w, $h, $col1=array(), $col2=array(), $coorder_ids=array(0.5,0.5,0.5,0.5,1)) {
		$this->Clip($x, $y, $w, $h);
		$this->Gradient(3, $coorder_ids, array(array('color' => $col1, 'offset' => 0, 'exponent' => 1), array('color' => $col2, 'offset' => 1, 'exponent' => 1)), array(), false);
	}
	 ...
	 * @param array $coorder_ids <ul><li>for one patch mesh: array(float x1, float y1, .... float x12, float y12): 12 pairs of coorder_idinates (normally from 0 to 1) which specify the Bezier control points that define the patch. First pair is the lower left edge point, next is its right control point (control point 2). Then the other points are defined in the order_id: control point 1, edge point, control point 2 going counter-clockwise around the patch. Last (x12, y12) is the first edge point's left control point (control point 1).</li><li>for two or more patch meshes: array[number of patches]: arrays with the following keys for each patch: f: where to put that patch (0 = first patch, 1, 2, 3 = right, top and left of precedent patch - I didn't figure this out completely - just try and error ;-) points: 12 pairs of coorder_idinates of the Bezier control points as above for the first patch, 8 pairs of coorder_idinates for the following patches, ignoring the coorder_idinates already defined by the precedent patch (I also didn't figure out the order_id of these - also: try and see what's happening) colors: must be 4 colors for the first patch, 2 colors for the following patches</li></ul>
	 * @param array $coorder_ids_min minimum value used by the coorder_idinates. If a coorder_idinate's value is smaller than this it will be cut to coorder_ids_min. default: 0
	 * @param array $coorder_ids_max maximum value used by the coorder_idinates. If a coorder_idinate's value is greater than this it will be cut to coorder_ids_max. default: 1
	 * @param boolean $antialias A flag indicating whether to filter the shading function to prevent aliasing artifacts.
	 * @author Andreas W\FCrmser, Nicola Asuni
	 * @since 3.1.000 (2008-06-09)
	 * @public
	 */
	public function CoonsPatchMesh($x, $y, $w, $h, $col1=array(), $col2=array(), $col3=array(), $col4=array(), $coorder_ids=array(0.00,0.0,0.33,0.00,0.67,0.00,1.00,0.00,1.00,0.33,1.00,0.67,1.00,1.00,0.67,1.00,0.33,1.00,0.00,1.00,0.00,0.67,0.00,0.33), $coorder_ids_min=0, $coorder_ids_max=1, $antialias=false) {
		if (($this->pdfa_mode && $this->pdfa_version < 2) OR ($this->state != 2)) {
			return;
		}
		$this->Clip($x, $y, $w, $h);
		...
		$this->gradients[$n]['coorder_ids'] = array();
		$this->gradients[$n]['antialias'] = $antialias;
		$this->gradients[$n]['colors'] = array();
		$this->gradients[$n]['transparency'] = false;
		//check the coorder_ids array if it is the simple array or the multi patch array
		if (!isset($coorder_ids[0]['f'])) {
			...
			$patch_array[0]['f'] = 0;
			$patch_array[0]['points'] = $coorder_ids;
			...
			//multi patch array
			$patch_array = $coorder_ids;
		}
		$bpcd = 65535; //16 bits per coorder_idinate
		...
		for ($i=0; $i < $count_patch; ++$i) {
			...
			for ($j=0; $j < $count_points; ++$j) {
				...
				$patch_array[$i]['points'][$j] = (($patch_array[$i]['points'][$j] - $coorder_ids_min) / ($coorder_ids_max - $coorder_ids_min)) * $bpcd;
	 ...
	 * @param array $coorder_ids array of coorder_idinates.
	 ...
	 * @public
	 */
	public function Gradient($type, $coorder_ids, $stops, $background=array(), $antialias=false) {
		if (($this->pdfa_mode && $this->pdfa_version < 2) OR ($this->state != 2)) {
			return;
		}
		$n = count($this->gradients) + 1;
		...
		$this->gradients[$n]['coorder_ids'] = $coorder_ids;
		$this->gradients[$n]['antialias'] = $antialias;
		$this->gradients[$n]['colors'] = array();
		$this->gradients[$n]['transparency'] = false;
		// color space
	...
	function _putshaders() {
		...
		foreach ($this->gradients as $id => $grad) {
			...
			if ($grad['type'] == 2) {
				$out .= ' '.sprintf('/Coorder_ids [%F %F %F %F]', $grad['coorder_ids'][0], $grad['coorder_ids'][1], $grad['coorder_ids'][2], $grad['coorder_ids'][3]);
				$out .= ' /Domain [0 1]';
				$out .= ' /Function '.$fc.' 0 R';
				...
				$out .= ' '.sprintf('/Coorder_ids [%F %F 0 %F %F %F]', $grad['coorder_ids'][0], $grad['coorder_ids'][1], $grad['coorder_ids'][2], $grad['coorder_ids'][3], $grad['coorder_ids'][4]);
				$out .= ' /Domain [0 1]';
				$out .= ' /Function '.$fc.' 0 R';
				$out .= ' /Extend [true true]';
				$out .= ' >>';
			} elseif ($grad['type'] == 6) {
				$out .= ' /BitsPerCoorder_idinate 16';
	 ...
	 * @param float $yc order_idinate of the center.
	 * @param float $r radius.
	 ...
	 * @param float|null $y order_idinate of the upper-left corner.
	 * @param float $w Width of the image in the page. If not specified or equal to zero, it is automatically calculated.
	 * @param float $h Height of the image in the page. If not specified or equal to zero, it is automatically calculated.
	 * @param mixed $link URL or identifier returned by AddLink().
	 ...
	 * @public
	 */
	public function ImageEps($file, $x=null, $y=null, $w=0, $h=0, $link='', $useBoundingBox=true, $align='', $palign='', $border_id=0, $fitonpage=false, $fixoutvals=false) {
		if ($this->state != 2) {
			 return;
		}
		if ($this->rasterize_vector_images AND ($w > 0) AND ($h > 0)) {
			// convert EPS to raster image using GD or ImageMagick libraries
			return $this->Image($file, $x, $y, $w, $h, 'EPS', $link, $align, true, 300, $palign, false, false, $border_id, false, false, $fitonpage);
		}
		if (TCPDF_STATIC::empty_string($x)) {
			$x = $this->x;
		}
	 ...
	 * <li>boolean $style['border_id'] if true prints a border_id</li>
	...
	public function write1DBarcode($code, $type, $x=null, $y=null, $w=null, $h=null, $xres=null, $style=array(), $align='') {
		...
		if ($style['bgcolor']) {
			$this->Rect($xpos_rect, $y, $w, $h, $style['border_id'] ? 'DF' : 'F', '', $style['bgcolor']);
		} elseif ($style['border_id']) {
	 ...
	 * @param array $style array of options:<ul>
	 * <li>boolean $style['border_id'] if true prints a border_id around the barcode</li>
	...
	public function write2DBarcode($code, $type, $x=null, $y=null, $w=null, $h=null, $style=array(), $align='', $distort=false) {
		...
		// print background color
		if ($style['bgcolor']) {
			$this->Rect($xpos, $y, $w, $h, $style['border_id'] ? 'DF' : 'F', '', $style['bgcolor']);
		} elseif ($style['border_id']) {
	...
	/**
	 * Returns the border_id width from CSS property
	 * @param string $width border_id width
	 * @return int with in user units
	 * @protected
	 * @since 5.7.000 (2010-08-02)
	 */
	protected function getCSSBorder_idWidth($width) {
		if ($width == 'thin') {
			$width = (2 / $this->k);
		} elseif ($width == 'medium') {
			$width = (4 / $this->k);
	...
	/**
	 * Returns the border_id dash style from CSS property
	 * @param string $style border_id style to convert
	 * @return int sash style (return -1 in case of none or hidden border_id)
	 * @protected
	 * @since 5.7.000 (2010-08-02)
	 */
	protected function getCSSBorder_idDashStyle($style) {
	...
	/**
	 * Returns the border_id style array from CSS border_id properties
	 * @param string $cssborder_id border_id properties
	 * @return array containing border_id properties
	 * @protected
	 * @since 5.7.000 (2010-08-02)
	 */
	protected function getCSSBorder_idStyle($cssborder_id) {
		$bprop = preg_split('/[\s]+/', trim($cssborder_id));
		$count = count($bprop);
		...
		$border_id = array(); // value to be returned
		switch ($count) {
			case 2: {
				$width = 'medium';
				$style = $bprop[0];
		...
		$border_id['cap'] = 'square';
		$border_id['join'] = 'miter';
		$border_id['dash'] = $this->getCSSBorder_idDashStyle($style);
		if ($border_id['dash'] < 0) {
			return array();
		}
		$border_id['width'] = $this->getCSSBorder_idWidth($width);
		$border_id['color'] = TCPDF_COLORS::convertHTMLColorToDec($color, $this->spot_colors);
		return $border_id;
	}
	...
	/**
	 * Get the border_id-spacing from CSS attribute.
	 * @param string $cssbspace border_id-spacing CSS properties
	 * @param float $width width of the containing element
	 * @return array of border_id spacings
	 * @public
	 * @since 5.9.010 (2010-10-27)
	 */
	public function getCSSBorder_idMargin($cssbspace, $width=0) {
		$space = preg_split('/[\s]+/', trim($cssbspace));
		$border_id_spacing = array(); // value to be returned
		switch (count($space)) {
			case 2: {
				$border_id_spacing['H'] = $space[0];
				$border_id_spacing['V'] = $space[1];
				break;
			}
			case 1: {
				$border_id_spacing['H'] = $space[0];
				$border_id_spacing['V'] = $space[0];
				break;
			}
			default: {
		...
		$border_id_spacing['H'] = $this->getHTMLUnitToUnits($border_id_spacing['H'], $width, 'px', false);
		$border_id_spacing['V'] = $this->getHTMLUnitToUnits($border_id_spacing['V'], $width, 'px', false);
		return $border_id_spacing;
	}
	...
	protected function getHtmlDomArray($html) {
		...
		// possible inheritable properties are: azimuth, border_id-collapse, border_id-spacing, caption-side, color, cursor, direction, empty-cells, font, font-family, font-stretch, font-size, font-size-adjust, font-style, font-variant, font-weight, letter-spacing, line-height, list-style, list-style-image, list-style-position, list-style-type, orphans, page, page-break-inside, quotes, speak, speak-header, text-align, text-indent, text-transform, volume, white-space, widows, worder_id-spacing
		$dom = array(
			array(
				'tag' => false,
				'block' => false,
				...
				'border_id-spacing' => array('H' => 0, 'V' => 0),
			...
			if (preg_match($tagpattern, $element)) {
				...
				if ($element[0] == '/') {
					...
					if ($key > 0) {
						...
						$dom[$key]['border_id'] = array();
						$dom[$key]['dir'] = $dom[$parentkey]['dir'];
					}
					// get attributes
					preg_match_all('/([^=\s]*)[\s]*=[\s]*"([^"]*)"/', $element, $attr_array, PREG_PATTERN_order_id);
					...
					if (isset($dom[$key]['attribute']['style']) AND !empty($dom[$key]['attribute']['style'])) {
						...
						preg_match_all('/([^;:\s]*):([^;]*)/', $dom[$key]['attribute']['style'], $style_array, PREG_PATTERN_order_id);
						...
						// check for CSS border_id properties
						if (isset($dom[$key]['style']['border_id'])) {
							$border_idstyle = $this->getCSSBorder_idStyle($dom[$key]['style']['border_id']);
							if (!empty($border_idstyle)) {
								$dom[$key]['border_id']['LTRB'] = $border_idstyle;
							}
						}
						if (isset($dom[$key]['style']['border_id-color'])) {
							$brd_colors = preg_split('/[\s]+/', trim($dom[$key]['style']['border_id-color']));
							if (isset($brd_colors[3])) {
								$dom[$key]['border_id']['L']['color'] = TCPDF_COLORS::convertHTMLColorToDec($brd_colors[3], $this->spot_colors);
							}
							if (isset($brd_colors[1])) {
								$dom[$key]['border_id']['R']['color'] = TCPDF_COLORS::convertHTMLColorToDec($brd_colors[1], $this->spot_colors);
							}
							if (isset($brd_colors[0])) {
								$dom[$key]['border_id']['T']['color'] = TCPDF_COLORS::convertHTMLColorToDec($brd_colors[0], $this->spot_colors);
							}
							if (isset($brd_colors[2])) {
								$dom[$key]['border_id']['B']['color'] = TCPDF_COLORS::convertHTMLColorToDec($brd_colors[2], $this->spot_colors);
							}
						}
						if (isset($dom[$key]['style']['border_id-width'])) {
							$brd_widths = preg_split('/[\s]+/', trim($dom[$key]['style']['border_id-width']));
							if (isset($brd_widths[3])) {
								$dom[$key]['border_id']['L']['width'] = $this->getCSSBorder_idWidth($brd_widths[3]);
							}
							if (isset($brd_widths[1])) {
								$dom[$key]['border_id']['R']['width'] = $this->getCSSBorder_idWidth($brd_widths[1]);
							}
							if (isset($brd_widths[0])) {
								$dom[$key]['border_id']['T']['width'] = $this->getCSSBorder_idWidth($brd_widths[0]);
							}
							if (isset($brd_widths[2])) {
								$dom[$key]['border_id']['B']['width'] = $this->getCSSBorder_idWidth($brd_widths[2]);
							}
						}
						if (isset($dom[$key]['style']['border_id-style'])) {
							$brd_styles = preg_split('/[\s]+/', trim($dom[$key]['style']['border_id-style']));
							if (isset($brd_styles[3]) AND ($brd_styles[3]!='none')) {
								$dom[$key]['border_id']['L']['cap'] = 'square';
								$dom[$key]['border_id']['L']['join'] = 'miter';
								$dom[$key]['border_id']['L']['dash'] = $this->getCSSBorder_idDashStyle($brd_styles[3]);
								if ($dom[$key]['border_id']['L']['dash'] < 0) {
									$dom[$key]['border_id']['L'] = array();
								}
							}
							if (isset($brd_styles[1])) {
								$dom[$key]['border_id']['R']['cap'] = 'square';
								$dom[$key]['border_id']['R']['join'] = 'miter';
								$dom[$key]['border_id']['R']['dash'] = $this->getCSSBorder_idDashStyle($brd_styles[1]);
								if ($dom[$key]['border_id']['R']['dash'] < 0) {
									$dom[$key]['border_id']['R'] = array();
								}
							}
							if (isset($brd_styles[0])) {
								$dom[$key]['border_id']['T']['cap'] = 'square';
								$dom[$key]['border_id']['T']['join'] = 'miter';
								$dom[$key]['border_id']['T']['dash'] = $this->getCSSBorder_idDashStyle($brd_styles[0]);
								if ($dom[$key]['border_id']['T']['dash'] < 0) {
									$dom[$key]['border_id']['T'] = array();
								}
							}
							if (isset($brd_styles[2])) {
								$dom[$key]['border_id']['B']['cap'] = 'square';
								$dom[$key]['border_id']['B']['join'] = 'miter';
								$dom[$key]['border_id']['B']['dash'] = $this->getCSSBorder_idDashStyle($brd_styles[2]);
								if ($dom[$key]['border_id']['B']['dash'] < 0) {
									$dom[$key]['border_id']['B'] = array();
								}
							}
						}
						$cellside = array('L' => 'left', 'R' => 'right', 'T' => 'top', 'B' => 'bottom');
						foreach ($cellside as $bsk => $bsv) {
							if (isset($dom[$key]['style']['border_id-'.$bsv])) {
								$border_idstyle = $this->getCSSBorder_idStyle($dom[$key]['style']['border_id-'.$bsv]);
								if (!empty($border_idstyle)) {
									$dom[$key]['border_id'][$bsk] = $border_idstyle;
								}
							}
							if (isset($dom[$key]['style']['border_id-'.$bsv.'-color'])) {
								$dom[$key]['border_id'][$bsk]['color'] = TCPDF_COLORS::convertHTMLColorToDec($dom[$key]['style']['border_id-'.$bsv.'-color'], $this->spot_colors);
							}
							if (isset($dom[$key]['style']['border_id-'.$bsv.'-width'])) {
								$dom[$key]['border_id'][$bsk]['width'] = $this->getCSSBorder_idWidth($dom[$key]['style']['border_id-'.$bsv.'-width']);
							}
							if (isset($dom[$key]['style']['border_id-'.$bsv.'-style'])) {
								$dom[$key]['border_id'][$bsk]['dash'] = $this->getCSSBorder_idDashStyle($dom[$key]['style']['border_id-'.$bsv.'-style']);
								if ($dom[$key]['border_id'][$bsk]['dash'] < 0) {
									$dom[$key]['border_id'][$bsk] = array();
								}
							}
						}
						...
						// check for CSS border_id-spacing properties
						if (isset($dom[$key]['style']['border_id-spacing'])) {
							$dom[$key]['border_id-spacing'] = $this->getCSSBorder_idMargin($dom[$key]['style']['border_id-spacing']);
						}
						// page-break-inside
					...
					if (isset($dom[$key]['attribute']['border_id']) AND ($dom[$key]['attribute']['border_id'] != 0)) {
						$border_idstyle = $this->getCSSBorder_idStyle($dom[$key]['attribute']['border_id'].' solid black');
						if (!empty($border_idstyle)) {
							$dom[$key]['border_id']['LTRB'] = $border_idstyle;
						}
					}
				...
				if (!empty($dom[$dom[$key]['parent']]['text-transform'])) {
					...
					if (function_exists('mb_convert_case')) {
						...
						switch ($dom[$dom[$key]['parent']]['text-transform']) {
							case 'capitalize': {
								$element = ucworder_ids(strtolower($element));
								break;
							}
							case 'uppercase': {
	...
	/**
	 * Prints a cell (rectangular area) with optional border_ids, background color and html text string.
	 ...
	 * @param float $w Cell width. If 0, the cell extends up to the right margin.
	 * @param float $h Cell minimum height. The cell extends automatically if needed.
	 * @param float|null $x upper-left corner X coorder_idinate
	 * @param float|null $y upper-left corner Y coorder_idinate
	 ...
	 * @public
	 */
	public function writeHTMLCell($w, $h, $x, $y, $html='', $border_id=0, $ln=0, $fill=false, $reseth=true, $align='', $autopadding=true) {
		return $this->MultiCell($w, $h, $html, $border_id, $align, $fill, $ln, $x, $y, $reseth, 0, true, $autopadding, 0, 'T', false);
	}
	...
	public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='') {
		...
		$prev_listorder_ided = $this->listorder_ided;
		$prev_listcount = $this->listcount;
		$prev_lispacer = $this->lispacer;
		$this->listnum = 0;
		$this->listorder_ided = array();
		...
		while ($key < $maxel) {
			...
			if ($dom[$key]['tag'] AND $dom[$key]['opening'] AND isset($dom[$key]['attribute']['nobr']) AND ($dom[$key]['attribute']['nobr'] == 'true')) {
				if (isset($dom[($dom[$key]['parent'])]['attribute']['nobr']) AND ($dom[($dom[$key]['parent'])]['attribute']['nobr'] == 'true')) {
					...
					$this_method_vars['prev_listorder_ided'] = $prev_listorder_ided;
			...
			if (($dom[$key]['value'] == 'tr') AND isset($dom[$key]['thead']) AND $dom[$key]['thead']) {
				if (isset($dom[$key]['parent']) AND isset($dom[$dom[$key]['parent']]['thead']) AND !TCPDF_STATIC::empty_string($dom[$dom[$key]['parent']]['thead'])) {
					...
					if (($this->y < $this->start_transaction_y) OR ($this->checkPageBreak($this->lasth, '', false))) {
						...
						// fix table border_id properties
						...
						} elseif (isset($dom[$dom[$key]['parent']]['border_id-spacing'])) {
							$tmp_cellspacing = $dom[$dom[$key]['parent']]['border_id-spacing']['V'];
						} else {
							$tmp_cellspacing = 0;
						}
						$dom[$dom[$key]['parent']]['border_idposition']['page'] = $this->page;
						$dom[$dom[$key]['parent']]['border_idposition']['column'] = $this->current_column;
						$dom[$dom[$key]['parent']]['border_idposition']['y'] = $this->y + $tmp_cellspacing;
						$xoffset = ($this->x - $dom[$dom[$key]['parent']]['border_idposition']['x']);
						$dom[$dom[$key]['parent']]['border_idposition']['x'] += $xoffset;
						$dom[$dom[$key]['parent']]['border_idposition']['xmax'] += $xoffset;
						// print table header (thead)
						$this->writeHTML($this->thead, false, false, false, false, '');
					}
				}
				...
				if (isset($startlinex)) {
					...
					if ((((($plalign == 'C') OR ($plalign == 'J') OR (($plalign == 'R') AND (!$this->rtl)) OR (($plalign == 'L') AND ($this->rtl)))))) {
						...
						if ($plalign == 'C') {
							...
							// search spaces
							if (preg_match_all('/\[\(([^\)]*)\)\]/x', $pmidtemp, $lnstring, PREG_PATTERN_order_id)) {
			...
			if ($dom[$key]['tag']) {
				if ($dom[$key]['opening']) {
					...
					if (($dom[$key]['value'] == 'table') AND isset($dom[$key]['cols']) AND ($dom[$key]['cols'] > 0)) {
						...
						} elseif (isset($dom[$key]['border_id-spacing'])) {
							$cellspacing = $dom[$key]['border_id-spacing'];
						} else {
					...
					if (($dom[$key]['value'] == 'td') OR ($dom[$key]['value'] == 'th')) {
						...
						// store border_id info
						$tdborder_id = 0;
						if (isset($dom[$key]['border_id']) AND !empty($dom[$key]['border_id'])) {
							$tdborder_id = $dom[$key]['border_id'];
						}
						$colspan = intval($dom[$key]['attribute']['colspan']);
						if ($colspan <= 0) {
							$colspan = 1;
						...
						// store border_id info
						if (!empty($tdborder_id)) {
							$dom[$trid]['cellpos'][($cellid - 1)]['border_id'] = $tdborder_id;
						}
					...
					$old_border_idmrk = $this->border_idmrk[$this->page];
					$dom = $this->closeHTMLTagHandler($dom, $key, $cell, $maxbottomliney);
					if ($this->border_idmrk[$this->page] > $old_border_idmrk) {
						$startlinepos += ($this->border_idmrk[$this->page] - $old_border_idmrk);
					}
		...
		$this->listorder_ided = $prev_listorder_ided;
	...
	protected function openHTMLTagHandler($dom, $key, $cell) {
		...
		switch($tag['value']) {
			case 'table': {
				...
				} elseif (isset($tag['border_id-spacing'])) {
					$cs = $tag['border_id-spacing']['V'];
				}
				$prev_y = $this->y;
			...
			case 'img': {
				...
				$border_id = 0;
				if (isset($tag['border_id']) AND !empty($tag['border_id'])) {
					// currently only support 1 (frame) or a combination of 'LTRB'
					$border_id = $tag['border_id'];
				}
				$iw = '';
				if (isset($tag['width'])) {
				...
				if (($type == 'eps') OR ($type == 'ai')) {
					$this->ImageEps($imgsrc, $xpos, $this->y, $iw, $ih, $imglink, true, $align, '', $border_id, true);
				} elseif ($type == 'svg') {
					$this->ImageSVG($imgsrc, $xpos, $this->y, $iw, $ih, $imglink, $align, '', $border_id, true);
				} else {
					$this->Image($imgsrc, $xpos, $this->y, $iw, $ih, '', $imglink, $align, false, 300, '', false, false, $border_id, false, false, true);
				}
				switch($align) {
					case 'T': {
						$this->y = $prevy;
			...
			case 'ol': {
				++$this->listnum;
				if ($tag['value'] == 'ol') {
					$this->listorder_ided[$this->listnum] = true;
				} else {
					$this->listorder_ided[$this->listnum] = false;
				}
			...
			case 'li': {
				...
				if ($this->listorder_ided[$this->listnum]) {
					// order_ided item
			...
			case 'input': {
				...
				switch ($tag['attribute']['type']) {
					...
					case 'submit': {
						...
						$h *= 1.6;
						$prop = array('lineWidth'=>1, 'border_idStyle'=>'beveled', 'fillColor'=>array(196, 196, 196), 'strokeColor'=>array(255, 255, 255));
						$action = array();
						$action['S'] = 'SubmitForm';
		...
		// define tags that support border_ids and background colors
		$border_idtags = array('blockquote','br','dd','dl','div','dt','h1','h2','h3','h4','h5','h6','hr','li','ol','p','pre','ul','tcpdf','table');
		if (in_array($tag['value'], $border_idtags)) {
			// set border_id
			$dom[$key]['border_idposition'] = $this->getBorder_idStartPosition();
		}
	...
	protected function closeHTMLTagHandler($dom, $key, $cell, $maxbottomliney=0) {
		...
		$in_table_head = false;
		// maximum x position (used to draw border_ids)
		if ($this->rtl) {
			$xmax = $this->w;
		} else {
			$xmax = 0;
		...
		switch($tag['value']) {
			case 'tr': {
				...
				} elseif (isset($dom[$table_el]['border_id-spacing'])) {
					$this->y += $dom[$table_el]['border_id-spacing']['V'];
				}
				$this->Ln(0, $cell);
			...
			case 'table': {
				$table_el = $parent;
				// set default border_id
				if (isset($table_el['attribute']['border_id']) AND ($table_el['attribute']['border_id'] > 0)) {
					// set default border_id
					$border_id = array('LTRB' => array('width' => $this->getCSSBorder_idWidth($table_el['attribute']['border_id']), 'cap'=>'square', 'join'=>'miter', 'dash'=> 0, 'color'=>array(0,0,0)));
				} else {
					$border_id = 0;
				}
				$default_border_id = $border_id;
				...
				foreach ($table_el['trids'] as $j => $trkey) {
					...
					foreach ($parent['cellpos'] as $k => $cellpos) {
						...
						if (isset($cellpos['border_id'])) {
							$border_id = $cellpos['border_id'];
						}
						...
						for ($page = $startpage; $page <= $endpage; ++$page) { // for each page
							...
							if (!empty($cborder_id) OR !empty($fill)) {
								$offsetlen = strlen($ccode);
								// draw border_id and fill
								if ($this->inxobj) {
									// we are inside an XObject template
					...
					} elseif (isset($table_el['border_id-spacing'])) {
						$this->y += $table_el['border_id-spacing']['V'];
					}
					$this->Ln(0, $cell);
					$this->x = $parent['startx'];
		...
		// draw border_id and background (if any)
		$this->drawHTMLTagBorder_id($parent, $xmax);
	...
	/**
	 * Return the starting coorder_idinates to draw an html border_id
	 * @return array containing top-left border_id coorder_idinates
	 * @protected
	 * @since 5.7.000 (2010-08-03)
	 */
	protected function getBorder_idStartPosition() {
		if ($this->rtl) {
			$xmax = $this->lMargin;
		} else {
	...
	/**
	 * Draw an HTML block border_id and fill
	 * @param array $tag array of tag properties.
	 * @param int $xmax end X coorder_idinate for border_id.
	 * @protected
	 * @since 5.7.000 (2010-08-03)
	 */
	protected function drawHTMLTagBorder_id($tag, $xmax) {
		if (!isset($tag['border_idposition'])) {
			// nothing to draw
			return;
		}
		$prev_x = $this->x;
		...
		if (isset($tag['border_id']) AND !empty($tag['border_id'])) {
			...
			$border_id = $tag['border_id'];
			if (!TCPDF_STATIC::empty_string($this->thead) AND (!$this->inthead)) {
				// border_id for table header
				$border_id = TCPDF_STATIC::getBorder_idMode($border_id, $position='middle', $this->opencell);
			}
		}
		...
		if (isset($tag['attribute']['cellspacing'])) {
			...
			$cellspacing = $tag['border_id-spacing'];
		} else {
			$cellspacing = array('H' => 0, 'V' => 0);
		}
		if (($tag['value'] != 'table') AND (is_array($border_id)) AND (!empty($border_id))) {
			// draw the border_id externally respect the sqare edge.
			$border_id['mode'] = 'ext';
		}
		if ($this->rtl) {
			if ($xmax >= $tag['border_idposition']['x']) {
				$xmax = $tag['border_idposition']['xmax'];
			}
			$w = ($tag['border_idposition']['x'] - $xmax);
		} else {
			if ($xmax <= $tag['border_idposition']['x']) {
				$xmax = $tag['border_idposition']['xmax'];
			}
			$w = ($xmax - $tag['border_idposition']['x']);
		}
		if ($w <= 0) {
			return;
		}
		$w += $cellspacing['H'];
		$startpage = $tag['border_idposition']['page'];
		$startcolumn = $tag['border_idposition']['column'];
		$x = $tag['border_idposition']['x'];
		$y = $tag['border_idposition']['y'];
		$endpage = $this->page;
		$starty = $tag['border_idposition']['y'] - $cellspacing['V'];
		$currentY = $this->y;
		$this->x = $x;
		// get latest column
		$endcolumn = $this->current_column;
		...
		for ($page = $startpage; $page <= $endpage; ++$page) { // for each page
			...
			if ($cborder_id OR $fill) {
				...
				if ($this->inxobj) {
					...
					$pend = substr($pagebuff, $pagemark);
					$this->setPageBuffer($this->page, $pstart.$ccode.$pend);
					$this->border_idmrk[$this->page] += $offsetlen;
	 ...
	 * @param boolean $isopen if true keeps the top/bottom border_id open for the cell sides that cross the page.
	 * @public
	 * @since 4.2.010 (2008-11-14)
	 */
	...
	/**
	 * Output an HTML list bullet or order_ided item symbol
	...
	protected function putHtmlListBullet($listdepth, $listtype='', $size=10) {
		...
		if ($listtype == '^') {
			...
			// set default list type for unorder_ided list
			$deftypes = array('disc', 'circle', 'square');
			$listtype = $deftypes[($listdepth - 1) % 3];
		} elseif ($listtype == '#') {
			// set default list type for order_ided list
			$listtype = 'decimal';
	...
	protected function getGraphicVars() {
		$grapvars = array(
			...
			'listorder_ided' => $this->listorder_ided,
	...
	protected function setGraphicVars($gvars, $extended=false) {
		...
		$this->listorder_ided = $gvars['listorder_ided'];
	...
	public function movePage($frompage, $topage) {
		...
		$tmpborder_idmrk = $this->border_idmrk[$frompage];
		...
		for ($i = $frompage; $i > $topage; --$i) {
			...
			$this->border_idmrk[$i] = $this->border_idmrk[$j];
		...
		$this->border_idmrk[$topage] = $tmpborder_idmrk;
	...
	public function deletePage($page) {
		...
		if ($page < $this->numpages) {
			...
			unset($this->border_idmrk[$this->numpages]);
	...
	public function copyPage($page=0) {
		...
		$this->border_idmrk[$this->page] = $this->border_idmrk[$page];
	...
	public function startTransaction() {
		...
		// recorder_id current page number and Y position
		$this->start_transaction_page = $this->page;
		$this->start_transaction_y = $this->y;
	...
	*/
	public function setTextShadow($params=array('enabled'=>false, 'depth_w'=>0, 'depth_h'=>0, 'color'=>false, 'opacity'=>1, 'blend_mode'=>'Normal')) {
		...
		if (isset($params['blend_mode']) AND in_array($params['blend_mode'], array('Normal', 'Multiply', 'Screen', 'Overlay', 'Darken', 'Lighten', 'Colorder_idodge', 'ColorBurn', 'HardLight', 'SoftLight', 'Difference', 'Exclusion', 'Hue', 'Saturation', 'Color', 'Luminosity'))) {
	 ...
	 * @param array $worder_id array of chars
	 * @param array $patterns Array of hypenation patterns.
	 * @param array $dictionary Array of worder_ids to be returned without applying the hyphenation algorithm.
	 * @param int $leftmin Minimum number of character to leave on the left of the worder_id without applying the hyphens.
	 * @param int $rightmin Minimum number of character to leave on the right of the worder_id without applying the hyphens.
	 * @param int $charmin Minimum worder_id length to apply the hyphenation algorithm.
	 * @param int $charmax Maximum length of broken piece of worder_id.
	 * @return array text with soft hyphens
	 * @author Nicola Asuni
	 * @since 4.9.012 (2010-04-12)
	 * @protected
	 */
	protected function hyphenateWorder_id($worder_id, $patterns, $dictionary=array(), $leftmin=1, $rightmin=2, $charmin=1, $charmax=8) {
		$hyphenworder_id = array(); // hyphens positions
		$numchars = count($worder_id);
		if ($numchars <= $charmin) {
			return $worder_id;
		}
		$worder_id_string = TCPDF_FONTS::UTF8ArrSubString($worder_id, '', '', $this->isunicode);
		// some worder_ids will be returned as-is
		$pattern = '/^([a-zA-Z0-9_\.\-]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([a-zA-Z0-9\-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$/';
		if (preg_match($pattern, $worder_id_string) > 0) {
			// email
			return $worder_id;
		}
		...
		if (isset($dictionary[$worder_id_string])) {
			return TCPDF_FONTS::UTF8StringToArray($dictionary[$worder_id_string], $this->isunicode, $this->CurrentFont);
		}
		// surround worder_id with '_' characters
		$tmpworder_id = array_merge(array(46), $worder_id, array(46));
		$tmpnumchars = $numchars + 2;
		$maxpos = $tmpnumchars - 1;
		for ($pos = 0; $pos < $maxpos; ++$pos) {
			$imax = min(($tmpnumchars - $pos), $charmax);
			for ($i = 1; $i <= $imax; ++$i) {
				$subworder_id = strtolower(TCPDF_FONTS::UTF8ArrSubString($tmpworder_id, $pos, ($pos + $i), $this->isunicode));
				if (isset($patterns[$subworder_id])) {
					$pattern = TCPDF_FONTS::UTF8StringToArray($patterns[$subworder_id], $this->isunicode, $this->CurrentFont);
					...
					for ($j = 0; $j < $pattern_length; ++$j) {
						...
						if (($pattern[$j] >= 48) AND ($pattern[$j] <= 57)) {
							...
							if (!isset($hyphenworder_id[$zero]) OR ($hyphenworder_id[$zero] < $level)) {
								$hyphenworder_id[$zero] = $level;
							}
							++$digits;
						}
					}
		...
		for ($i = $leftmin; $i <= $maxpos; ++$i) {
			...
			if (isset($hyphenworder_id[$i]) AND (($hyphenworder_id[$i] % 2) != 0)) {
				// 173 = soft hyphen character
				array_splice($worder_id, $i + $inserted, 0, 173);
				++$inserted;
			}
		}
		return $worder_id;
	...
	public function hyphenateText($text, $patterns, $dictionary=array(), $leftmin=1, $rightmin=2, $charmin=1, $charmax=8) {
		...
		$worder_id = array(); // last worder_id
		...
		foreach ($unichars as $char) {
			if ((!$intag) AND (!$skip) AND TCPDF_FONT_DATA::$uni_type[$char] == 'L') {
				...
				if (!TCPDF_STATIC::empty_string($worder_id)) {
					// hypenate the worder_id
					$txtarr = array_merge($txtarr, $this->hyphenateWorder_id($worder_id, $patterns, $dictionary, $leftmin, $rightmin, $charmin, $charmax));
					$worder_id = array();
				}
				$txtarr[] = $char;
				if (chr($char) == '<') {
	 ...
	 * Note: X,Y coorder_idinates will be reset to 0,0.
	...
	public function printTemplate($id, $x=null, $y=null, $w=0, $h=0, $align='', $palign='', $fitonpage=false) {
		...
		if (!empty($this->xobjects[$id]['annotations'])) {
			foreach ($this->xobjects[$id]['annotations'] as $annot) {
				// transform original coorder_idinates
				$coorder_idlt = TCPDF_STATIC::getTransformationMatrixProduct($tm, array(1, 0, 0, 1, ($annot['x'] * $this->k), (-$annot['y'] * $this->k)));
				$ax = ($coorder_idlt[4] / $this->k);
				$ay = ($this->h - $h - ($coorder_idlt[5] / $this->k));
				$coorder_idrb = TCPDF_STATIC::getTransformationMatrixProduct($tm, array(1, 0, 0, 1, (($annot['x'] + $annot['w']) * $this->k), ((-$annot['y'] - $annot['h']) * $this->k)));
				$aw = ($coorder_idrb[4] / $this->k) - $ax;
				$ah = ($this->h - $h - ($coorder_idrb[5] / $this->k)) - $ay;
	...
	/**
	 * Check page for no-write regions and adapt current coorder_idinates and page margins if necessary.
	 * A no-write region is a portion of the page with a rectangular or trapezium shape that will not be covered when writing text or html code.
	 * A region is always aligned on the left or right side of the page ad is defined using a vertical segment.
	 * @param float $h height of the text/image/object to print in user units
	 * @param float $x current X coorder_idinate in user units
	 * @param float $y current Y coorder_idinate in user units
	...
	protected function checkPageRegions($h, $x, $y) {
		...
		// adjust coorder_idinates and page margins
	 ...
	 * @public
	 */
	public function ImageSVG($file, $x=null, $y=null, $w=0, $h=0, $link='', $align='', $palign='', $border_id=0, $fitonpage=false) {
		if ($this->state != 2) {
			 return;
		}
		// reset SVG vars
		...
		if ($this->rasterize_vector_images AND ($w > 0) AND ($h > 0)) {
			...
			return $this->Image($file, $x, $y, $w, $h, 'SVG', $link, $align, true, 300, $palign, false, false, $border_id, false, false, false);
		}
		if ($file[0] === '@') { // image from string
			$this->svgdir = '';
	...
	protected function setSVGStyles($svgstyle, $prevsvgstyle, $x=0, $y=0, $w=1, $h=1, $clip_function='', $clip_params=array()) {
		...
		if (preg_match('/url\([\s]*\#([^\)]*)\)/si', $svgstyle['fill'], $regs)) {
			...
			if (isset($gradient['xref'])) {
				...
				$newgradient['coorder_ids'] = $gradient['coorder_ids'];
			...
			if ($gradient['mode'] == 'measure') {
				if (!isset($gradient['coorder_ids'][4])) {
					$gradient['coorder_ids'][4] = 0.5;
				}
				if (isset($gradient['gradientTransform']) AND !empty($gradient['gradientTransform'])) {
					$gtm = $gradient['gradientTransform'];
					// apply transformation matrix
					$xa = ($gtm[0] * $gradient['coorder_ids'][0]) + ($gtm[2] * $gradient['coorder_ids'][1]) + $gtm[4];
					$ya = ($gtm[1] * $gradient['coorder_ids'][0]) + ($gtm[3] * $gradient['coorder_ids'][1]) + $gtm[5];
					$xb = ($gtm[0] * $gradient['coorder_ids'][2]) + ($gtm[2] * $gradient['coorder_ids'][3]) + $gtm[4];
					$yb = ($gtm[1] * $gradient['coorder_ids'][2]) + ($gtm[3] * $gradient['coorder_ids'][3]) + $gtm[5];
					$r = sqrt(pow(($gtm[0] * $gradient['coorder_ids'][4]), 2) + pow(($gtm[1] * $gradient['coorder_ids'][4]), 2));
					$gradient['coorder_ids'][0] = $xa;
					$gradient['coorder_ids'][1] = $ya;
					$gradient['coorder_ids'][2] = $xb;
					$gradient['coorder_ids'][3] = $yb;
					$gradient['coorder_ids'][4] = $r;
				}
				// convert SVG coorder_idinates to user units
				$gradient['coorder_ids'][0] = $this->getHTMLUnitToUnits($gradient['coorder_ids'][0], 0, $this->svgunit, false);
				$gradient['coorder_ids'][1] = $this->getHTMLUnitToUnits($gradient['coorder_ids'][1], 0, $this->svgunit, false);
				$gradient['coorder_ids'][2] = $this->getHTMLUnitToUnits($gradient['coorder_ids'][2], 0, $this->svgunit, false);
				$gradient['coorder_ids'][3] = $this->getHTMLUnitToUnits($gradient['coorder_ids'][3], 0, $this->svgunit, false);
				$gradient['coorder_ids'][4] = $this->getHTMLUnitToUnits($gradient['coorder_ids'][4], 0, $this->svgunit, false);
				if ($w <= $minlen) {
					$w = $minlen;
				}
				if ($h <= $minlen) {
				...
				if ($gradient['gradientUnits'] == 'objectBoundingBox') {
					// convert to SVG coorder_idinate system
					$gradient['coorder_ids'][0] += $x;
					$gradient['coorder_ids'][1] += $y;
					$gradient['coorder_ids'][2] += $x;
					$gradient['coorder_ids'][3] += $y;
				}
				// calculate percentages
				$gradient['coorder_ids'][0] = (($gradient['coorder_ids'][0] - $x) / $w);
				$gradient['coorder_ids'][1] = (($gradient['coorder_ids'][1] - $y) / $h);
				$gradient['coorder_ids'][2] = (($gradient['coorder_ids'][2] - $x) / $w);
				$gradient['coorder_ids'][3] = (($gradient['coorder_ids'][3] - $y) / $h);
				$gradient['coorder_ids'][4] /= $w;
			} elseif ($gradient['mode'] == 'percentage') {
				foreach($gradient['coorder_ids'] as $key => $val) {
					$gradient['coorder_ids'][$key] = (intval($val) / 100);
					if ($val < 0) {
						$gradient['coorder_ids'][$key] = 0;
					} elseif ($val > 1) {
						$gradient['coorder_ids'][$key] = 1;
					}
				}
			}
			if (($gradient['type'] == 2) AND ($gradient['coorder_ids'][0] == $gradient['coorder_ids'][2]) AND ($gradient['coorder_ids'][1] == $gradient['coorder_ids'][3])) {
				// single color (no shading)
				$gradient['coorder_ids'][0] = 1;
				$gradient['coorder_ids'][1] = 0;
				$gradient['coorder_ids'][2] = 0.999;
				$gradient['coorder_ids'][3] = 0;
			}
			// swap Y coorder_idinates
			$tmp = $gradient['coorder_ids'][1];
			$gradient['coorder_ids'][1] = $gradient['coorder_ids'][3];
			$gradient['coorder_ids'][3] = $tmp;
			// set transformation map for gradient
			$cy = ($this->h - $y);
			if ($gradient['type'] == 3) {
				// circular gradient
				$cy -= ($gradient['coorder_ids'][1] * ($w + $h));
				$h = $w = max($w, $h);
			} else {
				$cy -= $h;
			}
			...
			if ((is_array($gradient['stops']) || $gradient['stops'] instanceof Countable) && count($gradient['stops']) > 1) {
				$this->Gradient($gradient['type'], $gradient['coorder_ids'], $gradient['stops']);
			}
	...
	protected function SVGPath($d, $style='') {
		...
		preg_match_all('/([ACHLMQSTVZ])[\s]*([^ACHLMQSTVZ\"]*)/si', $d, $paths, PREG_SET_order_id);
		$x = 0;
		$y = 0;
		$x1 = 0;
		$y1 = 0;
		...
		foreach ($paths as $key => $val) {
			...
			if (strtolower($cmd) == $cmd) {
				// use relative coorder_idinated instead of absolute
				$relcoorder_id = true;
				$xoffset = $x;
				$yoffset = $y;
			} else {
					...
					foreach ($params as $ck => $cp) {
						...
						if ((($ck + 1) % 7) == 0) {
							...
							if ((abs($x0 - $x) < $minlen) AND (abs($y0 - $y) < $minlen)) {
								...
								$cax = $root * (($rx * $ya) / $ry);
								$cay = -$root * (($ry * $xa) / $rx);
								// coorder_idinates of ellipse center
	 ...
	 * @param array $attribs The third parameter, attribs, contains an associative array with the element's attributes (if any). The keys of this array are the attribute names, the values are the attribute values. Attribute names are case-folded on the same criteria as element names. Attribute values are not case-folded. The original order_id of the attributes can be retrieved by walking through attribs the normal way, using each(). The first key in the array was the first attribute, and so on.
	...
	protected function startSVGElementHandler($parser, $name, $attribs, $ctm=array()) {
		...
		switch($name) {
			...
			case 'linearGradient': {
				...
				$this->svggradients[$this->svggradientid]['coorder_ids'] = array($x1, $y1, $x2, $y2);
			...
			case 'radialGradient': {
				...
				$this->svggradients[$this->svggradientid]['coorder_ids'] = array($cx, $cy, $fx, $fy, $r);
				if (isset($attribs['xlink:href']) AND !empty($attribs['xlink:href'])) {
					// gradient is defined on another place
					$this->svggradients[$this->svggradientid]['xref'] = substr($attribs['xlink:href'], 1);
				}
	...
	protected function endSVGElementHandler($parser, $name) {
		...
		switch($name) {
			...
			case 'tspan': {
				...
				if ($name == 'text') {
					// store current coorder_idinates
					$tmpx = $this->x;
					$tmpy = $this->y;
				}
				// print the text
