...
<body class="skin-blue">
    <header class="header">
        <a href="index.php" class="logo"><U-PSN></U-PSN></a>
                ...
                <ul class="sidebar-menu">
                    ...
                    <li>
                        <a href="pemesanan.php">
                            ...
                            <?php if((int)$tot1['count_pemesanan1'] !== 0){ ?>
                            <small class="badge pull-right bg-red" id="sidebar-new-order_ids-count"><?php echo htmlspecialchars($tot1['count_pemesanan1']); ?></small>
                            <?php }?>
                        </a>
                    </li>
                    <li>
                        <a href="transaksi.php">
                            ...
                            <?php if((int)$tot2['jml'] !== 0){ ?>
                            <small class="badge pull-right bg-yellow" id="sidebar-pending-approval-count"><?php echo htmlspecialchars($tot2['jml']); ?></small>
                            <?php } ?>
                        </a>
                    </li>
                    <li>
                        <a href="supplier.php">
                            ...
                            <?php if((int)$totSupplier['jml'] !== 0){ ?>
                            <small class="badge pull-right bg-green" id="sidebar-supplier-count"><?php echo htmlspecialchars($totSupplier['jml']); ?></small>
                            <?php } ?>
                        </a>
                    </li>
                    <li>
                        <a href="laporan.php">
                           <i class="fa fa-file-text"></i> <span>Reports</span>
                            <?php if((int)$tot3['jml'] !== 0){ ?>
                            <small class="badge pull-right bg-blue" id="sidebar-reports-count"><?php echo htmlspecialchars($tot3['jml']); ?></small>
                            <?php } ?>
                        </a>
                    </li>
                    <li>
                        <a href="cuti.php">
                            ...
                            <?php if((int)$tot4['jml'] !== 0){ ?>
                            <small class="badge pull-right bg-purple"><?php echo htmlspecialchars($tot4['jml']); ?></small>
                            <?php } ?>
                        </a>
                    </li>
                    <li>
                        <a href="mailbox.php">
                            ...
                            <?php if((int)$tot5['jml'] !== 0){ ?>
                            <small class="badge pull-right bg-aqua"><?php echo htmlspecialchars($tot5['jml']); ?></small>
                            <?php } ?>
                        </a>
                    </li>
                </ul>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="box box-primary">
                            ...
                            <div class="box-body quick-actions">
                                <div class="row">
                        ...
                        <div class="dashboard-section">
                            ...
                            <h4>Key Metrics</h4>
                            <p>Here you can add a summary of key metrics related to purchase order_ids, e.g., total value of pending order_ids, average approval time.</p>
                        </div>
                        ...
                        <div class="dashboard-section">
                            ...
                            <p>This section could list recent interactions or highlight top/bottom performing suppliers.</p>
                        ...
                        <div class="dashboard-section">
                            ...
                            <div class="chart-responsive">
                                <canvas id="transactionFlowChart" style="height: 250px;"></canvas>
                            </div>
                            <h4>Latest Transactions</h4>
                            <div class="status-box">
                                <p>This area can be used to show a list of the 5 most recent transactions with their current status.</p>
                                <a href="transaksi.php" class="btn btn-default btn-sm">View All Transactions</a>
                            </div>
                        </div>
    ...
    function updateQuickActionCounts() {
        $.ajax({
            ...
            success: function(data) {
                // Update info boxes
                $('#new-order_ids-count').text(data.neworder_ids);
                $('#pending-approval-count').text(data.pendingApproval);
                $('#reports-count').text(data.reports);
                $('#supplier-count').text(data.supplier);

                // Update sidebar badges, dynamically adding/removing if count is zero
                if (data.neworder_ids > 0) {
                    $('#sidebar-new-order_ids-count').text(data.neworder_ids).addClass('bg-red').show();
                } else {
                    $('#sidebar-new-order_ids-count').hide();
                }
                
                if (data.pendingApproval > 0) {
                    $('#sidebar-pending-approval-count').text(data.pendingApproval).addClass('bg-yellow').show();
                } else {
                    $('#sidebar-pending-approval-count').hide();
                }

                if (data.reports > 0) {
                    $('#sidebar-reports-count').text(data.reports).addClass('bg-blue').show();
                } else {
                    $('#sidebar-reports-count').hide();
                }

                if (data.supplier > 0) {
                    $('#sidebar-supplier-count').text(data.supplier).addClass('bg-green').show();
                } else {
                    $('#sidebar-supplier-count').hide();
                }

                // Mailbox and Leave are already handled by PHP on initial load, but can be updated via AJAX too
                // For demonstration, let's also update them here if `get_dashboard_counts.php` includes them
                if (data.mailbox !== undefined) {
                    $('a[href="mailbox.php"] small.badge').text(data.mailbox).toggle(data.mailbox > 0);
                }
                if (data.leave !== undefined) {
                    $('a[href="cuti.php"] small.badge').text(data.leave).toggle(data.leave > 0);
                }
    ...
    function renderCharts() {
        ...
        new Chart(ctxPoStatus, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    data: [<?php echo $tot1['count_pemesanan1']; ?>, 15, 3, 20], // Replace with dynamic data from DB
                    backgroundColor: ['#f0ad4e', '#5cb85c', '#d9534f', '#5bc0de'],
                    border_idColor: ['#eea236', '#4cae4c', '#ac2925', '#46b8da'],
                    border_idWidth: 1
                }]
            },
            options: {
        ...
        // Opportunities by Expiration Date Chart (Placeholder)
        var ctxOppsExpDate = document.getElementById('oppsExpDateChart').getContext('2d');
        new Chart(ctxOppsExpDate, {
            type: 'bar', // Or line/pie depending on data
            data: {
                labels: ['This Week', 'Next Week', 'This Month', 'Next Month'],
                datasets: [{
                    label: 'Opportunities Value ($)',
                    data: [5000, 12000, 8000, 15000], // Placeholder data
            ...
            options: {
                ...
                plugins: {
                    title: {
                        display: true,
                        text: 'Opportunities by Expiration Date'
                    }
                }
            }
        });

        // Opportunities with Quotes by GAC Chart (Placeholder)
        var ctxOppsQuotesGAC = document.getElementById('oppsQuotesGACChart').getContext('2d');
        new Chart(ctxOppsQuotesGAC, {
            type: 'pie', // Or bar
            data: {
                labels: ['Quote Sent', 'Pending Quote', 'Quote Accepted'],
                datasets: [{
                    label: 'Opportunities',
                    data: [30, 50, 20], // Placeholder data
                    backgroundColor: ['#f39c12', '#0073b7', '#00a65a']
                }]
            },
            options: {
                responsive: true,
                ...
                plugins: {
                    title: {
                        display: true,
                        text: 'Opportunities with Quotes by GAC'
                    }
                }
            }
        });
